...
static int SV_FindLeafFolders(const char* baseFolder, char* result, int maxResults, int maxFolderLength) {
	char* fileList = static_cast<char*>(Z_Malloc(MAX_OSPATH * maxResults, TAG_FILESYS)); // too big for stack since this is recursive
	...
	for (int i = 0; i < numFiles; i++) {
		if (Q_stricmp(fileName, ".") && Q_stricmp(fileName, "..")) {
			...
			const int newResults = SV_FindLeafFolders(fullFolder, nextResult, maxResults - resultCount, maxFolderLength);
...
void SV_BeginAutoRecordDemos() {
	if (sv_autoDemo->integer) {
		...
		if (sv_autoDemoMaxMaps->integer > 0 && sv.demosPruned == qfalse) {
			...
			const int autorecordDirListCount = SV_FindLeafFolders("demos/autorecord", autorecordDirList, 500, MAX_OSPATH);
