...
int CSequencer::ParseLoop(CBlock* block, bstream_t* bstream)
{
	...
	{
		sequence->SetIterations(static_cast<int>(*(float*)bm->GetData()));
	}
...
int CSequencer::ParseAffect(CBlock* block, bstream_t* bstream)
{
	...
	if (!ent) // if there wasn't a valid entname in the affect, we need to check if it's a get command
	{
		...
		{
			//get( TYPE, NAME )
			const int type = static_cast<int>(*(float*)block->GetMemberData(1));
...
void CSequencer::CheckRun(CBlock** command)
{
	...
	{
		const int id = static_cast<int>(*(float*)block->GetMemberData(1));
...
int CSequencer::EvaluateConditional(CBlock* block) const
{
	...
	case ID_GET:
	{
		...
		//get( TYPE, NAME )
		type = static_cast<int>(*(float*)block->GetMemberData(memberNum++));
...
void CSequencer::CheckIf(CBlock** command)
{
	...
	if (block->GetBlockID() == ID_IF)
	{
		...
		if (ret /*TRUE*/)
		{
			...
			{
				successID = static_cast<int>(*(float*)block->GetMemberData(block->GetNumMembers() - 2));
			}
			else
			{
				successID = static_cast<int>(*(float*)block->GetMemberData(block->GetNumMembers() - 1));
			}
		...
		{
			const int failureID = static_cast<int>(*(float*)block->GetMemberData(block->GetNumMembers() - 1));
...
void CSequencer::CheckLoop(CBlock** command)
{
	...
	if (block->GetBlockID() == ID_LOOP)
	{
		...
		{
			iterations = static_cast<int>(*(float*)bm->GetData());
		}

		const int loopID = static_cast<int>(*(float*)block->GetMemberData(memberNum++));
...
void CSequencer::CheckAffect(CBlock** command)
{
	...
	if (block->GetBlockID() == ID_AFFECT)
	{
		...
		if (!ent) // if there wasn't a valid entname in the affect, we need to check if it's a get command
		{
			...
			{
				//get( TYPE, NAME )
				const int type = static_cast<int>(*(float*)block->GetMemberData(memberNum++));
		...
		const int	type = static_cast<int>(*(float*)block->GetMemberData(memberNum));
		const int	id = static_cast<int>(*(float*)block->GetMemberData(memberNum + 1));
