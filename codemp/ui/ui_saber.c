...
{
	return WP_SaberParseParm(saber_name, "saberModel", saber_model);
}

qboolean UI_SaberSkinForSaber(const char* saber_name, char* saber_skin)
{
	return WP_SaberParseParm(saber_name, "customSkin", saber_skin);
}
...
qboolean UI_SaberProperNameForSaber(const char* saber_name, char* saber_proper_name)
{
	...
	{
		trap->SE_GetStringTextString(&stringedSaberName[1], saber_proper_name, 1024);
	}
	else
	{
		// no stringed so just use it as it
		strcpy(saber_proper_name, stringedSaberName);
	}
