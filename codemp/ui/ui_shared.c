...
static unsigned hashForString(const char* str) {
	...
	int i = 0;
	while (str[i] != '\0') {
		const char letter = tolower((unsigned char)str[i]);
...
qboolean Float_Parse(char** p, float* f) {
	const char* token = COM_ParseExt((const char**)p, qfalse);
	if (token && token[0] != 0) {
		*f = atof(token);
		return qtrue;
	}
...
qboolean Script_SetItemColor(itemDef_t* item, char** args)
{
	...
	if (String_Parse(args, &itemname) && String_Parse(args, &name))
	{
		...
		const int count = Menu_ItemsMatchingGroup((menuDef_t*)item->parent, itemname);
		...
		{
			itemDef_t* item2 = Menu_GetMatchingItemByNumber((menuDef_t*)item->parent, j, itemname);
			if (item2 != NULL)
			{
				vec4_t* out = NULL;
				if (Q_stricmp(name, "backcolor") == 0)
...
qboolean Script_Scale(itemDef_t* item, char** args)
{
	...
	if (String_Parse(args, &name))
	{
		...
		const int count = Menu_ItemsMatchingGroup((menuDef_t*)item->parent, name);

		if (Float_Parse(args, &scale))
		{
			for (int j = 0; j < count; j++)
			{
				const itemDef_t* itemFound = Menu_GetMatchingItemByNumber((menuDef_t*)item->parent, j, name);
				if (itemFound != NULL)
				{
...
qboolean Script_SetFocus(itemDef_t* item, char** args)
{
	...
	if (String_Parse(args, &name)) {
		itemDef_t* focusItem = Menu_FindItemByName((menuDef_t*)item->parent, name);
...
static void Scroll_Slider_ThumbFunc(void* p) {
	float x;
	const scrollInfo_t* si = (scrollInfo_t*)p;
...
void Item_Bind_Paint(itemDef_t* item)
{
	...
	if (item->text)
	{
		...
		float textWidth = DC->textWidth(g_nameBind, (float)textScale, item->iMenuFont);
...
qboolean ItemParse_maxChars(itemDef_t* item, int handle) {
	Item_ValidateTypeData(item);
	editFieldDef_t* editPtr = (editFieldDef_t*)item->typeData.edit;
