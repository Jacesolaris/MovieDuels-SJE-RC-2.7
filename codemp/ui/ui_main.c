...
int UI_ParseAnimationFile(const char* filename, animation_t* animset, qboolean isHumanoid)
{
	...
	while (1)
	{
		const char* token = COM_Parse((const char**)(&text_p));
...
void UI_FindCurrentSiegeTeamClass(void)
{
	...
	if (myTeam == TEAM_RED)
	{
		itemDef_t* item = (itemDef_t*)Menu_FindItemByName(menu, "onteam1");
		if (item)
		{
			Item_RunScript(item, item->action);
		}
	...
	{
		itemDef_t* item = (itemDef_t*)Menu_FindItemByName(menu, "onteam2");
		if (item)
		{
			Item_RunScript(item, item->action);
		}
...
static void UI_SetSaberBoxesandHilts(void)
{
	...
	itemDef_t* item = (itemDef_t*)Menu_FindItemByName(menu, "box2middle");
...
static void UI_ResetCharacterListBoxes(void)
{
	...
	if (menu)
	{
		itemDef_t* item = (itemDef_t*)Menu_FindItemByName((menuDef_t*)menu, "headlistbox");
		if (item)
		{
...
static qhandle_t UI_FeederItemImage(float feederID, int index) {
	...
	if (feederID == FEEDER_Q3HEADS)
	{
		...
		{ //we want it to load them as it draws them, like the TA feeder
			...
			{ //this isn't the best way of doing this I guess, but I didn't want a whole seperate string array
				...
				int i = strlen(uiInfo.q3HeadNames[index]);
	...
	else if (feederID == FEEDER_SIEGE_CLASS_FORCE)
	{
		...
		if (menu)
		{
			const itemDef_t* item = (itemDef_t*)Menu_FindItemByName((menuDef_t*)menu, "base_class_force_feed");
			if (item)
			{
...
static qboolean GameType_Parse(char** p, qboolean join) {
	const char* token = COM_ParseExt((const char**)p, qtrue);
...
static void UI_BuildPlayerModel_List(qboolean inGameLoad)
{
	...
	for (int i = 0; i < numdirs; i++, dirptr += dirlen + 1)
	{
		...
		if (f)
		{
			...
			const int numfiles = trap->FS_GetFileList(va("models/players/%s", dirptr), ".skin", filelist, sizeof(filelist));
			char* fileptr = filelist;
