...
void NPC_RemoveBody(gentity_t* ent)
{
		...
		|| ent->client->NPC_class == CLASS_MARK2)
	{
		...
		if (!trap->ICARUS_IsRunning(ent->s.number))
		{
			if (!ent->activator || !ent->activator->client || !(ent->activator->client->ps.eFlags2 & EF2_HELD_BY_MONSTER))
	...
	//		|| self->client->playerTeam == NPCTEAM_HIROGEN || self->client->playerTeam == NPCTEAM_MALON )
		...
		if (ent->enemy)
		{
			...
			if (!trap->ICARUS_IsRunning(ent->s.number))
			{
				if (!ent->activator || !ent->activator->client || !(ent->activator->client->ps.eFlags2 & EF2_HELD_BY_MONSTER))
				{//not being held by a Rancor
					if (ent->client && ent->client->ps.saberEntityNum > 0 && ent->client->ps.saberEntityNum < ENTITYNUM_WORLD)
					{
...
void NPC_BehaviorSet_Charmed(int b_state)
{
	switch (b_state)
	{
...
void NPC_BehaviorSet_Default(int b_state)
{
	switch (b_state)
	{
...
-------------------------
*/
void NPC_BehaviorSet_Interrogator(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_ImperialProbe(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Seeker(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Remote(int b_state)
{
...
-------------------------
*/
void NPC_BehaviorSet_Sentry(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Grenadier(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Sniper(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
void NPC_BehaviorSet_Jedi(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Droid(int b_state)
{
	switch (b_state)
	{
	case BS_DEFAULT:
...
-------------------------
*/
void NPC_BehaviorSet_Mark1(int b_state)
{
	switch (b_state)
	{
	case BS_DEFAULT:
...
-------------------------
*/
void NPC_BehaviorSet_Mark2(int b_state)
{
	switch (b_state)
	{
	case BS_DEFAULT:
...
-------------------------
*/
void NPC_BehaviorSet_ATST(int b_state)
{
	switch (b_state)
	{
	case BS_DEFAULT:
...
-------------------------
*/
void NPC_BehaviorSet_MineMonster(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Howler(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
-------------------------
*/
void NPC_BehaviorSet_Rancor(int b_state)
{
	switch (b_state)
	{
	case BS_STAND_GUARD:
...
void NPC_RunBehavior(int team, int b_state)
{
	...
	else if (jedi_cultist_destroyer(NPCS.NPC))
	{
		npc_bs_jedi_default();
	}
	...
	else
	{
		...
		case NPCTEAM_ENEMY:
			...
			if (NPCS.NPC->enemy && NPCS.NPC->s.weapon == WP_NONE && b_state != BS_HUNT_AND_KILL && !trap->ICARUS_TaskIDPending((sharedEntity_t*)NPCS.NPC, TID_MOVE_NAV))
...
void NPC_Think(gentity_t* ent)//, int msec )
{
	...
	if (d_npcfreeze.value || (NPCS.NPC->r.svFlags & SVF_ICARUS_FREEZE))
	{
		...
		ClientThink(ent->s.number, &NPCS.ucmd);
		//VectorCopy(self->s.origin, self->s.origin2 );
		VectorCopy(ent->r.currentOrigin, ent->client->ps.origin);
		return;
	}
	...
	if (ent->client->NPC_class == CLASS_VEHICLE)
	{
		...
		memcpy(&ent->m_pVehicle->m_ucmd, &ent->client->pers.cmd, sizeof(usercmd_t));
	}
