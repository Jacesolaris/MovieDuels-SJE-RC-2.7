...
int Q3_PlaySound(int taskID, int entID, const char* name, const char* channel)
{
	...
	const int soundHandle = G_SoundIndex((char*)finalName);
...
static qboolean Q3_SetBState(int entID, const char* bs_name)
{
	...
	const bState_t bSID = (bState_t)(GetIDForString(BSTable, bs_name));
	if (bSID != (bState_t)-1)
	{
...
void UnLockDoors(gentity_t* ent);
void LockDoors(gentity_t* ent);
