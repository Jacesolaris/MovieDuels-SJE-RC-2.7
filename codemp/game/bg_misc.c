...
qboolean BG_LegalizedForcePowers(char* powerOut, size_t powerOutSize, int maxRank, qboolean freeSaber, int teamForce, int gametype, int fpDisabled)
{
	...
	const int allowedPoints = forceMasteryPoints[maxRank];
	...
	while (i < NUM_FORCE_POWERS)
	{
		int countDown = Com_Clampi(0, NUM_FORCE_POWER_LEVELS, final_Powers[i]);
	...
	*/
		{
			"weapon_jango",
			"sound/weapons/w_pkup.wav",
								{ "models/weapons2/jango_pistol/briar_pistol_w.glm", 0, 0, 0},
								/* view */		"models/weapons2/jango_pistol/briar_pistol.md3",
