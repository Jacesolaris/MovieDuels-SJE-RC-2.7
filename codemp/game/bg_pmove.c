...
static float PM_CmdScale(usercmd_t* cmd) {
	...
	const float total = sqrt((float)(cmd->forwardmove * cmd->forwardmove
		+ cmd->rightmove * cmd->rightmove + umove * umove));
	const float scale = (float)pm->ps->speed * max / (127.0 * total);
		...
		BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
	{
		...
		{//on the ground
			...
			if (anim != -1)
			{
				...
				if (!doTrace || (trace.fraction < 1.0f && (trace.entity_num < MAX_CLIENTS || DotProduct(wallNormal, idealNormal) > 0.7)))
				{//there is a wall there.. or hit a client
					...
					{//wall-runs can only run on perfectly flat walls, sorry.
						...
						if ( doTrace && anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT )
						{
							...
							{
								pm->ps->forceKickFlip = trace.entity_num+1; //let the server know that this person gets kicked by this client
							}
						}
						*/
			...
			{//run up wall, flip backwards
				if (allowWallRuns)
				{
					...
					if (1) //sure, we have it! Because I SAID SO.
					{
						...
						if (trace.fraction < 1.0f
							&& ((trace.entity_num < ENTITYNUM_WORLD && trace_ent && trace_ent->s.solid != SOLID_BMODEL) || DotProduct(trace.plane.normal, idealNormal) > 0.7))
						{//there is a wall there
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
...
static void PM_FlyVehicleMove(void)
{
	...
	float	wishspeed;
	vec3_t	wishdir;
	const float	fmove = 0.0f, smove = 0.0f;
...
static void PM_CrashLand(void) {
	...
	const float dist = pm->ps->origin[2] - pml.previous_origin[2];
	const float vel = pml.previous_velocity[2];
	const float acc = -pm->ps->gravity;

	const float a = acc / 2;
	const float b = vel;
	const float c = -dist;

	const float den = b * b - 4 * a * c;
	if (den < 0) {
		pm->ps->inAirAnim = qfalse;
		return;
	}
	const float t = (-b - sqrt(den)) / (2 * a);
...
static void PM_Weapon(void)
{
	...
	if (pm->ps->client_num >= MAX_CLIENTS &&
		...
		pm_entSelf)
	{ //npc with no weapon
		const gentity_t* gent = (gentity_t*)pm_entSelf;
	...
	{ //check for exceeding max charge time if not using disruptor or rocket launcher or thermals
		if (pm->ps->weaponstate == WEAPON_CHARGING_ALT)
		{
			const int timeDif = (pm->cmd.serverTime - pm->ps->weaponChargeTime);
		...
		case HANDEXTEND_KNOCKDOWN:
			if (pm->ps->forceDodgeAnim)
			{
				...
				{ //this means that we want to play a sepereate anim on the torso
					const int originalDAnim = pm->ps->forceDodgeAnim - 8; //-8 is the original legs anim
					if (originalDAnim == 2)
					{
	...
	*/
	...
	{
		const int desTAnim = pm->ps->legsAnim;
	...
	vec3_t origin, vec3_t angles, vec3_t scale, int blend_time, qboolean forceHalt)
{
	...
	{
		const int baseposeAnim = basePose;
	...
	{ //we are a vehicle
		...
		else
		{
			...
			while (i < veh->m_pVehicle->m_iNumPassengers)
			{
				...
				{
					const gentity_t* thePassenger = (gentity_t*)veh->m_pVehicle->m_ppPassengers[i]; //yes, this is, in fact, ass.
