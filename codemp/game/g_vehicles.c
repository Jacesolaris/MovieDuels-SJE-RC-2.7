...
void G_VehicleTrace(trace_t* results, const vec3_t start, const vec3_t tMins, const vec3_t tMaxs, const vec3_t end, int pass_entity_num, int contentmask)
{
	trap->Trace(results, start, tMins, tMaxs, end, pass_entity_num, contentmask, qfalse, 0, 0);
}

Vehicle_t* G_IsRidingVehicle(gentity_t* p_ent)
{
...
qboolean VEH_TryEject(Vehicle_t* p_veh,
	gentity_t* parent,
	gentity_t* ent,
	int ejectDir,
	vec3_t vExitPos)
...
static void StartDeathDelay(Vehicle_t* p_veh, int iDelayTimeOverride)
{
	gentity_t* parent = (gentity_t*)p_veh->m_pParentEntity;
	...
	if (level.time >= p_veh->m_iDieTime)
	{
		...
		{ //explode now as long as we managed to kick everyone out
			...
			if (p_veh->m_pVehicleInfo->explosionRadius > 0 && p_veh->m_pVehicleInfo->explosionDamage > 0)
			{
				...
				G_RadiusDamage(trace.endpos, NULL, p_veh->m_pVehicleInfo->explosionDamage, p_veh->m_pVehicleInfo->explosionRadius, NULL, NULL, MOD_SUICIDE);//FIXME: extern damage and radius or base on fuel
			}
...
qboolean Initialize(Vehicle_t* p_veh)
{
	...
	parent->s.m_iVehicleNum = 0;
	{
		...
		parent->client->pers.maxHealth = parent->client->ps.stats[STAT_MAX_HEALTH] = parent->NPC->stats.health = parent->health = parent->client->ps.stats[STAT_HEALTH] = p_veh->m_iArmor;
	...
	for (i = 0; i < MAX_VEHICLE_TURRETS; i++)
	{
		...
		parent->client->ps.ammo[MAX_VEHICLE_WEAPONS + i] = p_veh->turretStatus[i].ammo = p_veh->m_pVehicleInfo->turret[i].iAmmoMax;
	...
	*/
	...
	{
		const int iFlags = SETANIM_FLAG_NORMAL, iBlend = 300;
		p_veh->m_ulFlags |= VEH_GEARSOPEN;
...
static qboolean Update(Vehicle_t* p_veh, const usercmd_t* pUmcd)
{
	...
	for (i = 0; i < MAX_VEHICLE_WEAPONS; i++)
	{
		if (p_veh->m_pVehicleInfo->weapon[i].ID > VEH_WEAPON_BASE//have a weapon in this slot
			...
			&& pUmcd->serverTime - p_veh->weaponStatus[i].lastAmmoInc >= p_veh->m_pVehicleInfo->weapon[i].ammoRechargeMS)//enough time has passed
		{//add 1 to the ammo
	...
	for (i = 0; i < MAX_VEHICLE_TURRETS; i++)
	{
		if (p_veh->m_pVehicleInfo->turret[i].iWeapon > VEH_WEAPON_BASE//have a weapon in this slot
			...
			&& pUmcd->serverTime - p_veh->turretStatus[i].lastAmmoInc >= p_veh->m_pVehicleInfo->turret[i].iAmmoRechargeMS)//enough time has passed
		{//add 1 to the ammo
...
maintainSelfDuringBoarding:
	...
	*/
		...
		)
	{
		...
		switch (shiftSound)
		{
		case 1: shiftSound = p_veh->m_pVehicleInfo->soundShift1; break;
		case 2: shiftSound = p_veh->m_pVehicleInfo->soundShift2; break;
		case 3: shiftSound = p_veh->m_pVehicleInfo->soundShift3; break;
		case 4: shiftSound = p_veh->m_pVehicleInfo->soundShift4; break;
		}
		if (shiftSound)
		{
...
static qboolean UpdateRider(Vehicle_t* p_veh, bgEntity_t* pRider, usercmd_t* pUmcd)
{
...
qboolean Inhabited(Vehicle_t* p_veh) { return (p_veh->m_pPilot || p_veh->m_iNumPassengers) ? qtrue : qfalse; }
