...
qboolean NPC_CheckEnemyStealth(gentity_t* target)
{
	...
	if (clearLOS)
	{
		...
		const float turning_rating = 5.0f;//AngleDelta( target->client->ps.viewangles[PITCH], target->lastAngles[PITCH] )/180.0f + AngleDelta( target->client->ps.viewangles[YAW], target->lastAngles[YAW] )/180.0f;
...
static qboolean NPC_ST_InvestigateEvent(int eventID, qboolean extraSuspicious)
{
	...
	*/
	...
	if (level.alertEvents[eventID].level > AEL_MINOR && NPCS.NPCInfo->investigateCount > 1 && (NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES))
	{
		...
		{
			const int id = NPC_FindCombatPoint(NPCS.NPCInfo->investigateGoal, NPCS.NPCInfo->investigateGoal, NPCS.NPCInfo->investigateGoal, CP_INVESTIGATE | CP_HAS_ROUTE, 0, -1);
