...
void NPC_BSHuntAndKill(void)
{
	...
	if (NPCS.NPC->enemy)
	{
		const visibility_t oEVis = NPCS.enemyVisibility = NPC_CheckVisibility(NPCS.NPC->enemy, CHECK_FOV | CHECK_SHOOT);//CHECK_360|//CHECK_PVS|
	...
	{//still don't have an enemy
		...
		{//check for alert events
			...
			{//heard/saw something
				...
				{//was a big event
					if (level.alertEvents[alert_event].owner && level.alertEvents[alert_event].owner->client && level.alertEvents[alert_event].owner->health >= 0 && level.alertEvents[alert_event].owner->client->playerTeam == NPCS.NPC->client->enemyTeam)
					{//an enemy
