...
{
	const char* token;
	int				num, n, lowestVal, highestVal, lastAnimEvent = 0;
	...
	while (1)
	{
		...
		int curAnimEvent = CheckAnimFrameForEventType(animEvents, keyFrame, eventType);
		if (curAnimEvent == -1)
...
int BG_ParseAnimationEvtFile(const char* as_filename, int animFileIndex, int eventFileIndex)
{
	...
	animation_t* animations = bgAllAnims[animFileIndex].anims;
...
int BG_ParseAnimationFile(const char* filename, animation_t* animset, qboolean isHumanoid)
{
	...
	while (1)
	{
		char* token = COM_Parse((const char**)(&text_p));
