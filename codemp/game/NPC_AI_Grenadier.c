...
static qboolean Grenadier_Move(void)
{
	...
	{//couldn't get to enemy
		...
		{//we were running after enemy
			...
			int cp = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentOrigin,
				cpFlags, 32, -1);
...
void NPC_BSGrenadier_Attack(void)
{
	...
	if (NPC_ClearLOS4(NPCS.NPC->enemy))
	{
		...
		{//in front of me
			...
			{
				VectorCopy(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPCInfo->enemyLastSeenLocation);
				const float enemyHorzDist = DistanceHorizontalSquared(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin);
