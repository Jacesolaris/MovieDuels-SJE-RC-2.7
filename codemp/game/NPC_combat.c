...
qboolean NPC_ReserveCombatPoint(int combat_point_id)
{
...
qboolean NPC_FreeCombatPoint(int combat_point_id, qboolean failed)
{
	if (failed)
	{//remember that this one failed for us
		NPCS.NPCInfo->lastFailedCombatPoint = combat_point_id;
	}
	//Make sure it's valid
	if (combat_point_id > level.numCombatPoints)
		return qfalse;
...
qboolean NPC_SetCombatPoint(int combat_point_id)
{
	...
	NPCS.NPCInfo->combatPoint = combat_point_id;
