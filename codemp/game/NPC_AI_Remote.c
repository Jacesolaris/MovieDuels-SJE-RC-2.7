...
void Remote_Attack(void)
{
	...
	const float idealDist = MIN_DISTANCE_SQR + (MIN_DISTANCE_SQR * flrand(0, 1));
	const qboolean advance = (qboolean)(distance > idealDist * 1.25);
	const qboolean retreat = (qboolean)(distance < idealDist * 0.75);
