...
//---------------------------------------------------------
{
	...
	if (altFire)
	{
		...
		const float boxSize = REY_ALT_SIZE * (count * 0.5);
...
//---------------------------------------------------------
{
	...
	if (altFire)
	{
		...
		const float boxSize = CLONEPISTOL_ALT_SIZE * (count * 0.5);
...
//---------------------------------------------------------
{
	...
	{//need to loop this in case we hit a Jedi who dodges the shot
		...
		if (trace_ent && trace_ent->client && trace_ent->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_3)
		{
			if (WP_SaberCanBlock(trace_ent, tr.endpos, 0, MOD_DISRUPTOR, qtrue, 0))
		...
		else if ((trace_ent->flags & FL_SHIELDED))
		{//stopped cold
			return;
		}
...
//---------------------------------------------------------
{
	...
	for (int i = 0; i < traces; i++)
	{
		...
		if (trace_ent && trace_ent->client && trace_ent->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_3)
		{
			if (WP_SaberCanBlock(trace_ent, tr.endpos, 0, MOD_DISRUPTOR_SNIPER, qtrue, 0))
			{ //broadcast and stop the shot because it was blocked
		...
		if (render_impact)
		{
			...
			else
			{
					...
					|| trace_ent->s.eType == ET_MOVER)
				{
					...
					{
						G_Damage(trace_ent, ent, ent, forward, tr.endpos, damage,
...
void DEMP2_AltRadiusDamage(gentity_t* ent)
{
	...
	float radius = frac * 200.0f; // 200 is max radius...the model is aprox. 100 units tall...the fx draw code mults. this by 2.
		...
		{//do a rough trace of the path
			...
			for (int elapsedTime = timeStep; elapsedTime < floor(travelTime) + timeStep; elapsedTime += timeStep)
			{
				...
				if (trace.fraction < 1.0f)
				{
					...
					if (trace.plane.normal[2] > 0.7 && DistanceSquared(trace.endpos, target) < 4096)//hit within 64 of desired location, should be okay
					{//close enough!
						break;
					}
...
void WP_DropDetPack(gentity_t* ent, qboolean altFire)
{
	gentity_t* found = NULL;
	int			trapcount = 0;
...
{//a rail-gun-like beam
	const int			damage = CONC_ALT_DAMAGE, traces = DISRUPTOR_ALT_TRACES;
	qboolean	render_impact = qtrue;
	vec3_t		start, end;
	vec3_t		/*muzzle2,*/ dir;
	trace_t		tr;
	...
	for (int i = 0; i < traces; i++)
	{
		...
		if (trace_ent->s.weapon == WP_SABER)//&& trace_ent->NPC
		{//FIXME: need a more reliable way to know we hit a jedi?
			hitDodged = Jedi_DodgeEvasion(trace_ent, ent, &tr, HL_NONE);
		...
		if (!hitDodged)
		{
			if (render_impact)
			{
					...
					|| trace_ent->s.eType == ET_MOVER)
				{
					...
					const qboolean noKnockBack = (trace_ent->flags & FL_NO_KNOCKBACK);//will be set if they die, I want to know if it was on *before* they died
					if (trace_ent && trace_ent->client && trace_ent->client->NPC_class == CLASS_GALAKMECH)
					{//hehe
						G_Damage(trace_ent, ent, ent, forward, tr.endpos, 10, DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_CONC_ALT);
						break;
					}
					G_Damage(trace_ent, ent, ent, forward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_CONC_ALT);
						...
						{//alive
							...
							{//knock-downable
								...
								if (pStr > 200.0f)
								{
									...
									trace_ent->client->ps.forceDodgeAnim = 0; //this toggles between 1 and 0, when it's 1 we should play the get up anim
								}
...
gentity_t* WP_FireVehicleWeapon(gentity_t* ent, vec3_t start, vec3_t dir, vehWeaponInfo_t* vehWeapon, qboolean altFire, qboolean isTurretWeap)
{
...
void WP_VehLeadCrosshairVeh(gentity_t* camtrace_ent, vec3_t newEnd, const vec3_t dir, const vec3_t shotStart, vec3_t shotDir)
{
	if (camtrace_ent
		&& camtrace_ent->client
...
qboolean WP_VehCheckTraceFromCamPos(gentity_t* ent, const vec3_t shotStart, vec3_t shotDir)
{
		...
		|| ent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER)
	{
		...
		trace_t	extraTrace;
		vec3_t	newEnd;
		const int camtrace_entNum = BG_VehTraceFromCamPos(&extraTrace, (bgEntity_t*)ent, ent->r.currentOrigin, shotStart, end, newEnd, shotDir, (trace.fraction * g_cullDistance));
		if (camtrace_entNum)
		{
...
//---------------------------------------------------------
void FireVehicleWeapon(gentity_t* ent, qboolean altFire)
//---------------------------------------------------------
{
	Vehicle_t* p_veh = ent->m_pVehicle;
	int muzzlesFired = 0;
	...
	if (p_veh->m_pVehicleInfo &&
		(p_veh->m_pVehicleInfo->type != VH_FIGHTER || (p_veh->m_ulFlags & VEH_WINGSOPEN))) // NOTE: Wings open also denotes that it has already launched.
		...
		for (i = 0; i < MAX_VEHICLE_MUZZLES; i++)
		{
			...
			if (p_veh->m_iMuzzleTag[i] != -1 && p_veh->m_iMuzzleWait[i] < level.time)
			{
				...
				{//have enough ammo to shoot
					...
					missile = WP_FireVehicleWeapon(ent, start, dir, vehWeapon, altFire, qfalse);
					if (vehWeapon->fHoming)
