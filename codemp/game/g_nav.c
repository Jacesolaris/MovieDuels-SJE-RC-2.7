...
void NPC_SetMoveGoal(const gentity_t* ent, vec3_t point, int radius, qboolean isNavGoal, int combatPoint, gentity_t* targetEnt)
{
	//Must be an NPC
	if (ent->NPC == NULL)
	{
...
qboolean NAV_ClearPathToPoint(gentity_t* self, vec3_t pmins, vec3_t pmaxs, vec3_t point, int clipmask, int okToHitEntNum)
{
	...
	if (self->flags & FL_NAVGOAL)
	{
		...
		{
			if (trace.entity_num < ENTITYNUM_WORLD && (&g_entities[trace.entity_num] != NULL) && g_entities[trace.entity_num].s.eType != ET_MOVER)
			{
				vec3_t	p1, p2;
...
int NAV_FindClosestWaypointForPoint(gentity_t* ent, vec3_t point)
{
	...
	const int bestWP = trap->Nav_GetNearestNode((sharedEntity_t*)marker, marker->waypoint, NF_CLEAR_PATH, WAYPOINT_NONE);
