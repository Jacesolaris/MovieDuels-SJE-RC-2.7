...
void NPC_BSFollowLeader(void)
{
	...
	//leader visible?
	const visibility_t leaderVis = NPC_CheckVisibility(NPCS.NPC->client->leader, CHECK_PVS | CHECK_360 | CHECK_SHOOT);//			ent->e_UseFunc = useF_NULL;
	...
	{//Don't move toward leader if we're in a full-body attack anim
		...
		const float leaderDist = VectorLength(vec);//FIXME: make this just nav distance?
		...
		{
			npc_move_dir_clear(NPCS.ucmd.forwardmove, NPCS.ucmd.rightmove, qtrue);
		}
	}
}
