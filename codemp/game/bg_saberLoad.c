...
void WP_RemoveSaber(saberInfo_t* sabers, int saber_num)
{
	...
	WP_SaberSetDefaults(&sabers[saber_num]);
...
void WP_SaberSetColor(saberInfo_t* sabers, int saber_num, int blade_num, char* colorName)
{
	if (!sabers)
	{
		return;
...
void WP_SaberLoadParms(void)
{
	...
	*marker = 0;
	...
	const int fileCnt = trap->FS_GetFileList("ext_data/sabers", ".sab", saberExtensionListBuf, sizeof(saberExtensionListBuf));
