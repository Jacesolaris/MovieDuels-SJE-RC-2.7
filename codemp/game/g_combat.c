...
void TossClientItems(gentity_t* self) {
	...
	if (weapon > WP_BRYAR_PISTOL &&
		...
		gentity_t* te = G_TempEntity(vec3_origin, EV_DESTROY_WEAPON_MODEL);
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
	...
	*/
	...
	{
		const int spawnTime = (level.clients[level.sortedClients[0]].respawnTime > level.clients[level.sortedClients[1]].respawnTime) ? level.clients[level.sortedClients[0]].respawnTime : level.clients[level.sortedClients[1]].respawnTime;
...
int RaySphereIntersections(vec3_t origin, float radius, vec3_t point, vec3_t dir, vec3_t intersections[2]) {
	...
	VectorNormalize(dir);
	const float b = 2 * (dir[0] * (point[0] - origin[0]) + dir[1] * (point[1] - origin[1]) + dir[2] * (point[2] - origin[2]));
...
void G_GetDismemberBolt(gentity_t* self, vec3_t boltPoint, int limbType)
{
	...
	trap->G2API_GetBoltMatrix(self->ghoul2, 0, useBolt, &bolt_matrix, properAngles, properOrigin, level.time, NULL, self->modelScale);

	boltPoint[0] = bolt_matrix.matrix[0][3];
	boltPoint[1] = bolt_matrix.matrix[1][3];
	boltPoint[2] = bolt_matrix.matrix[2][3];
	...
	{ //Make some saber hit sparks over the severed wrist area
		...
		boltAngles[0] = -bolt_matrix.matrix[0][1];
		boltAngles[1] = -bolt_matrix.matrix[1][1];
		boltAngles[2] = -bolt_matrix.matrix[2][1];
				...
				if (tagName)
				{
					...
					if (tagBolt != -1)
					{
						...
						trap->G2API_GetBoltMatrix(ent->ghoul2, 0, tagBolt,
							...
							actualTime, NULL, ent->modelScale);
						BG_GiveMeVectorFromMatrix(&bolt_matrix, ORIGIN, tagOrg);
						BG_GiveMeVectorFromMatrix(&bolt_matrix, NEGATIVE_Y, tagDir);
						if (DistanceSquared(point, tagOrg) < 256)
						{//hit close
...
qboolean G_RadiusDamage(vec3_t origin, gentity_t* attacker, float damage, float radius,
	...
	const qboolean	roastPeople = qfalse;
	...
	*/
	...
	for (int e = 0; e < num_listed_entities; e++) {
		...
		const float points = damage * (1.0 - dist / radius);
