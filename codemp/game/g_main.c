...
void RemoveDuelDrawLoser(void)
{
	...
	const int clFirst = level.clients[level.sortedClients[0]].ps.stats[STAT_HEALTH] + level.clients[level.sortedClients[0]].ps
		.stats[STAT_ARMOR];
	const int clSec = level.clients[level.sortedClients[1]].ps.stats[STAT_HEALTH] + level.clients[level.sortedClients[1]].ps.
		stats[STAT_ARMOR];
...
void AdjustTournamentScores(void) {
		...
		level.clients[level.sortedClients[1]].pers.connected == CON_CONNECTED)
	{
		...
		if (clFailure != 2)
		{
			client_num = level.sortedClients[clSuccess];
			...
			client_num = level.sortedClients[clFailure];
...
void G_RunFrame(int levelTime) {
	...
	if (gDoSlowMoDuel)
	{
		...
		{
			const float timeDif = (level.time - gSlowMoDuelTime); //difference in time between when the slow motion was initiated and now
