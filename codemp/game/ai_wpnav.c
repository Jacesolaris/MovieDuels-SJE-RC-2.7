...
{
	char* flagstr = (char*)B_TempAlloc(128);
	int i = 0;
...
int ConnectTrail(int startindex, int endindex, qboolean behindTheScenes)
{
	...
	while (nodenum < MAX_NODETABLE_SIZE && !foundit && cancontinue)
	{
		...
		{ //adjust the branch distance dynamically depending on the distance from the start and end points.
			...
			const float startDistf = VectorLength(startDist);
			const float endDistf = VectorLength(endDist);
		...
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			while (c < gWPNum)
			{
					...
					NotWithinRange(i, c))
				{
					...
					const int forceJumpable = CanForceJumpTo(i, c, nLDist);
...
void CalculateSiegeGoals(void)
{
	...
	while (i < level.num_entities)
	{
		...
		{
			tent = ent;
			gentity_t* t2ent = GetObjectThatTargets(tent);
			int looptracker = 0;
...
int LoadPathData(const char* filename)
{
	...
	char* routePath = (char*)B_TempAlloc(1024);
	...
	char* fileString = (char*)B_TempAlloc(524288);
	char* currentVar = (char*)B_TempAlloc(2048);
...
int AcceptBotCommand(char* cmd, gentity_t* pl)
{
	...
	const char* OptionalSArgument = NULL;
	const char* RequiredSArgument = NULL;
