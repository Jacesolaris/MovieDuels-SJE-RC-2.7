...
extern void WP_DeactivateSaber(const gentity_t* self, qboolean clear_length); //clearLength = qfalse
...
static void jedi_aggression_erosion(int amt)
{
...
static qboolean jedi_battle_taunt(void)
{
...
qboolean npc_move_dir_clear(int forwardmove, int rightmove, qboolean reset)
{
...
static void jedi_hold_position(void)
{
...
static void jedi_check_decrease_saber_anim_level(void)
{
...
static void jedi_combat_distance(int enemy_dist)
{//FIXME: for many of these checks, what we really want is horizontal distance to enemy
	...
	if (jedi_cultist_destroyer(NPCS.NPC))
	{
		Jedi_Advance();
...
static qboolean jedi_strafe(int strafe_time_min, int strafe_time_max, int next_strafe_time_min, int next_strafe_time_max, qboolean walking)
{
	if (jedi_cultist_destroyer(NPCS.NPC))
	{
		return qfalse;
	...
	if (TIMER_Done(NPCS.NPC, "strafeLeft") && TIMER_Done(NPCS.NPC, "strafeRight"))
	{
		...
		if (Q_irand(0, 1))
		{
			if (npc_move_dir_clear(NPCS.ucmd.forwardmove, -127, qfalse))
			{
				TIMER_Set(NPCS.NPC, "strafeLeft", strafeTime);
				strafed = qtrue;
			}
			else if (npc_move_dir_clear(NPCS.ucmd.forwardmove, 127, qfalse))
			{
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t* self, float rightdot, float zdiff)
{
	...
	{//already running on a wall
		...
		const float animLength = BG_AnimLength(self->localAnimIndex, (animNumber_t)self->client->ps.legsAnim);
...
evasionType_t Jedi_SaberBlockGo(gentity_t* self, usercmd_t* cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t* incoming, float dist) //dist = 0.0f
{
	...
	const float rightdot = DotProduct(right, diff);// + flrand(-0.10f,0.10f);
	//totalHeight = self->client->renderInfo.eyePoint[2] - self->r.absmin[2];
	const float zdiff = hitloc[2] - self->client->renderInfo.eyePoint[2];// + Q_irand(-6,6);
...
static qboolean Jedi_SaberBlock(int saber_num, int blade_num) //saber_num = 0, blade_num = 0
{
	...
	*/
	...
	float dist = ShortestLineSegBewteen2LineSegs(NPCS.NPC->enemy->client->renderInfo.muzzlePoint, saberTip, bottom, top,
...
-------------------------
*/
static void jedi_evasion_saber(vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir)
{
	vec3_t	dirEnemy2Me;
...
static void jedi_set_enemy_info(vec3_t enemy_dest, vec3_t enemy_dir, float* enemy_dist, vec3_t enemy_movedir, float* enemy_movespeed, int prediction)
{
...
static void Jedi_FaceEnemy(qboolean doPitch)
{
	...
	{//boba leads his enemy
		if (NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth * 0.5f)
		{//lead
			const float missileSpeed = WP_SpeedOfMissileForWeapon(NPCS.NPC->s.weapon, ((qboolean)(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)));
			if (missileSpeed)
			{
...
static void jedi_debounce_direction_changes(void)
{
...
static void jedi_timers_apply(void)
{
...
static void jedi_combat_timers_update(int enemy_dist)
{
	if (jedi_cultist_destroyer(NPCS.NPC))
	{
...
static void jedi_combat_idle(int enemy_dist)
{
	if (!TIMER_Done(NPCS.NPC, "parryTime"))
	{
		return;
...
static qboolean jedi_attack_decide(int enemy_dist)
{
...
static void jedi_check_enemy_movement(float enemy_dist)
{
	if (!NPCS.NPC->enemy || !NPCS.NPC->enemy->client)
	{
		return;
...
static void jedi_check_jumps(void)
{
	vec3_t	jumpVel;
	trace_t	trace;
	trajectory_t	tr;
...
static void jedi_combat(void)
{
	...
	{//not gripping
		...
		if (!jedi_clear_path_to_spot(enemy_dest, NPCS.NPC->enemy->s.number))
		{//hunt him down
	...
	*/
	...
	if (!npc_move_dir_clear(NPCS.ucmd.forwardmove, NPCS.ucmd.rightmove, qtrue))
...
qboolean jedi_check_ambush_player(void)
{
	float target_dist;
...
static void jedi_patrol(void)
{
	...
	if (jedi_waiting_ambush(NPCS.NPC))
...
-------------------------
*/
void npc_bs_jedi_follow_leader(void)
{
	NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
	if (!NPCS.NPC->enemy)
	{
	...
	{//saber is not in hand
		...
		{//
			...
			{//fell to the ground, try to pick it up...
				if (jedi_can_pull_back_saber(NPCS.NPC))
				{
...
qboolean jedi_in_special_move(void)
{
