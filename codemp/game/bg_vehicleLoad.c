...
static qboolean BG_ParseVehWeaponParm(vehWeaponInfo_t* vehWeapon, const char* parmName, char* pValue)
{
	...
	vehField_t* vehWeaponField = (vehField_t*)Q_LinearSearch(parmName, vehWeaponFields, numVehWeaponFields,
...
static qboolean BG_ParseVehicleParm(vehicleInfo_t* vehicle, const char* parmName, char* pValue)
{
	...
	// Loop through possible parameters
	vehField_t* vehField = (vehField_t*)Q_LinearSearch(parmName, vehicleFields, numVehicleFields,
...
void BG_VehWeaponLoadParms(void)
{
	...
	*marker = 0;
	...
	char* tempReadBuffer = (char*)BG_TempAlloc(MAX_VEH_WEAPON_DATA_SIZE);
...
{//HMM... only do this if there's a vehicle on the level?
	...
	*marker = 0;
	...
	//now load in the extra .veh extensions
	const int fileCnt = trap->FS_GetFileList("ext_data/vehicles", ".veh", vehExtensionListBuf, sizeof(vehExtensionListBuf));

	char* holdChar = vehExtensionListBuf;

	char* tempReadBuffer = (char*)BG_TempAlloc(MAX_VEHICLE_DATA_SIZE);
