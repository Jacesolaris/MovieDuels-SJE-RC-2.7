...
qboolean WP_SaberBladeUseSecondBladeStyle(saberInfo_t* saber, int blade_num);
qboolean WP_SaberBladeDoTransitionDamage(saberInfo_t* saber, int blade_num);
...
static QINLINE int G_SaberAttackPower(gentity_t* ent, qboolean attacking)
{
	...
	int baseLevel = ent->client->ps.fd.saber_anim_level;
...
static QINLINE qboolean SaberAttacking(gentity_t* self)
{
	...
	if (BG_SaberInSpecial(self->client->ps.saber_move))
		return qtrue;
...
int G_SaberLockAnim(int attacker_saber_style, int defender_saber_style, int top_or_side, int lock_or_break_or_super_break, int win_or_lose)
{
	int baseAnim = -1;
	...
	{//special case: if we're using the same style and locking
		if (attacker_saber_style == defender_saber_style
			|| (attacker_saber_style >= SS_FAST && attacker_saber_style <= SS_TAVION && defender_saber_style >= SS_FAST && defender_saber_style <= SS_TAVION))
		{//using same style
...
static QINLINE qboolean WP_SabersCheckLock2(gentity_t* attacker, gentity_t* defender, sabersLockMode_t lockMode)
{
	...
	{//use the new system
		...
		{//top lock
			attAnim = G_SaberLockAnim(attacker->client->ps.fd.saber_anim_level, defender->client->ps.fd.saber_anim_level, SABERLOCK_TOP, SABERLOCK_LOCK, SABERLOCK_WIN);
			defAnim = G_SaberLockAnim(defender->client->ps.fd.saber_anim_level, attacker->client->ps.fd.saber_anim_level, SABERLOCK_TOP, SABERLOCK_LOCK, SABERLOCK_LOSE);
			attStart = defStart = 0.5f;
		}
		else
		{//side lock
			...
			{
			case LOCK_DIAG_TR:
				attAnim = G_SaberLockAnim(attacker->client->ps.fd.saber_anim_level, defender->client->ps.fd.saber_anim_level, SABERLOCK_SIDE, SABERLOCK_LOCK, SABERLOCK_WIN);
				defAnim = G_SaberLockAnim(defender->client->ps.fd.saber_anim_level, attacker->client->ps.fd.saber_anim_level, SABERLOCK_SIDE, SABERLOCK_LOCK, SABERLOCK_LOSE);
				attStart = defStart = 0.5f;
				break;
			case LOCK_DIAG_TL:
				attAnim = G_SaberLockAnim(attacker->client->ps.fd.saber_anim_level, defender->client->ps.fd.saber_anim_level, SABERLOCK_SIDE, SABERLOCK_LOCK, SABERLOCK_LOSE);
				defAnim = G_SaberLockAnim(defender->client->ps.fd.saber_anim_level, attacker->client->ps.fd.saber_anim_level, SABERLOCK_SIDE, SABERLOCK_LOCK, SABERLOCK_WIN);
				attStart = defStart = 0.5f;
				break;
			case LOCK_DIAG_BR:
				attAnim = G_SaberLockAnim(attacker->client->ps.fd.saber_anim_level, defender->client->ps.fd.saber_anim_level, SABERLOCK_SIDE, SABERLOCK_LOCK, SABERLOCK_WIN);
	...
	attacker->client->ps.saberLockFrame = bgAllAnims[attacker->localAnimIndex].anims[attAnim].firstFrame + (bgAllAnims[attacker->localAnimIndex].anims[attAnim].num_frames * attStart);

	G_SetAnim(defender, NULL, SETANIM_BOTH, defAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
	defender->client->ps.saberLockFrame = bgAllAnims[defender->localAnimIndex].anims[defAnim].firstFrame + (bgAllAnims[defender->localAnimIndex].anims[defAnim].num_frames * defStart);
...
static QINLINE qboolean WP_GetSaberDeflectionAngle(gentity_t* attacker, gentity_t* defender, float saberHitFraction)
{
	...
	const int attSaberLevel = G_SaberAttackPower(attacker, SaberAttacking(attacker));
	const int defSaberLevel = G_SaberAttackPower(defender, SaberAttacking(defender));

	if (animBasedDeflection)
	{
		//Hmm, let's try just basing it off the anim
		const int attQuadStart = saberMoveData[attacker->client->ps.saber_move].startQuad;
		const int attQuadEnd = saberMoveData[attacker->client->ps.saber_move].endQuad;
		int defQuad = saberMoveData[defender->client->ps.saber_move].endQuad;
...
#ifndef FINAL_BUILD
			int attMove = attacker->client->ps.saber_move;
#endif
		...
		int newQuad = attQuadEnd + ceil(((float)quadDiff) / 2.0f);
		if (newQuad < Q_BR)
		...
		int attMove = attacker->client->ps.saber_move;
#endif
		attacker->client->ps.saber_move = PM_SaberDeflectionForQuad(newQuad);
#ifndef FINAL_BUILD
		if (g_saberDebugPrint.integer)
		{
	...
	{//a deflection
		...
		{//deflect to right
			if (swingUDot > 0.25f)
			{//deflect to top
				attacker->client->ps.saber_move = LS_D1_TR;
			}
			else if (swingUDot < -0.25f)
			{//deflect to bottom
				attacker->client->ps.saber_move = LS_D1_BR;
			}
			else
			{//deflect horizontally
				attacker->client->ps.saber_move = LS_D1__R;
			}
		}
		...
		{//deflect to left
			if (swingUDot > 0.25f)
			{//deflect to top
				attacker->client->ps.saber_move = LS_D1_TL;
			}
			else if (swingUDot < -0.25f)
			{//deflect to bottom
				attacker->client->ps.saber_move = LS_D1_BL;
			}
			else
			{//deflect horizontally
				attacker->client->ps.saber_move = LS_D1__L;
			}
		}
		else
		{//deflect in middle
			if (swingUDot > 0.25f)
			{//deflect to top
				attacker->client->ps.saber_move = LS_D1_T_;
			}
			else if (swingUDot < -0.25f)
			{//deflect to bottom
				attacker->client->ps.saber_move = LS_D1_B_;
			}
			else
...
static QINLINE int G_GetAttackDamage(gentity_t* self, int minDmg, int maxDmg, float multPoint)
{
	...
	float attackAnimLength = bgAllAnims[self->localAnimIndex].anims[self->client->ps.torsoAnim].num_frames * fabs((float)(bgAllAnims[self->localAnimIndex].anims[self->client->ps.torsoAnim].frameLerp));
	float animSpeedFactor = 1.0f;

	//Be sure to scale by the proper anim speed just as if we were going to play the animation
	BG_SaberStartTransAnim(self->s.number, self->client->ps.fd.saber_anim_level, self->client->ps.weapon, self->client->ps.torsoAnim, &animSpeedFactor, self->client->ps.brokenLimbs);
	const int speedDif = attackAnimLength - (attackAnimLength * animSpeedFactor);
	attackAnimLength += speedDif;
	float peakPoint = attackAnimLength;
	...
	const float currentPoint = self->client->ps.torsoTimer;

	float damageFactor = (float)((currentPoint / peakPoint));
	if (damageFactor > 1)
	{
...
static QINLINE float G_GetAnimPoint(gentity_t* self)
{
	...
	const float animPercentage = currentPoint / attackAnimLength;
...
{//return largest length
	...
	{
		const float len = WP_SaberBladeLength(&ent->client->saber[i]);
		if (len > bestLen)
		{
			bestLen = len;
		}
...
qboolean WP_SabersIntersect(gentity_t* ent1, int ent1saber_num, int ent1blade_num, gentity_t* ent2, qboolean checkDir)
{
	...
	for (int ent2saber_num = 0; ent2saber_num < MAX_SABERS; ent2saber_num++)
	{
		if (ent2->client->saber[ent2saber_num].type != SABER_NONE)
		{
			for (int ent2blade_num = 0; ent2blade_num < ent2->client->saber[ent2saber_num].numBlades; ent2blade_num++)
			{
				if (ent2->client->saber[ent2saber_num].blade[ent2blade_num].lengthMax > 0)
				{//valid saber and this blade is on
					//if ( ent1->client->saberInFlight )
					{
						VectorCopy(ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzlePointOld, saberBase1);
						VectorCopy(ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzlePoint, saberBaseNext1);

						VectorSubtract(ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzlePoint, ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzlePointOld, dir);
						VectorNormalize(dir);
						VectorMA(saberBaseNext1, SABER_EXTRAPOLATE_DIST, dir, saberBaseNext1);

						VectorMA(saberBase1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].lengthMax + SABER_EXTRAPOLATE_DIST, ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzleDirOld, saberTip1);
						VectorMA(saberBaseNext1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].lengthMax + SABER_EXTRAPOLATE_DIST, ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzleDir, saberTipNext1);
					...
					{
						VectorCopy( ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzlePoint, saberBase1 );
						VectorCopy( ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzlePointNext, saberBaseNext1 );
						VectorMA( saberBase1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].lengthMax, ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzleDir, saberTip1 );
						VectorMA( saberBaseNext1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].lengthMax, ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzleDirNext, saberTipNext1 );
					}
					*/

					//if ( ent2->client->saberInFlight )
					{
						VectorCopy(ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzlePointOld, saberBase2);
						VectorCopy(ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzlePoint, saberBaseNext2);

						VectorSubtract(ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzlePoint, ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzlePointOld, dir);
						VectorNormalize(dir);
						VectorMA(saberBaseNext2, SABER_EXTRAPOLATE_DIST, dir, saberBaseNext2);

						VectorMA(saberBase2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].lengthMax + SABER_EXTRAPOLATE_DIST, ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzleDirOld, saberTip2);
						VectorMA(saberBaseNext2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].lengthMax + SABER_EXTRAPOLATE_DIST, ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzleDir, saberTipNext2);
					...
					{
						VectorCopy( ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzlePoint, saberBase2 );
						VectorCopy( ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzlePointNext, saberBaseNext2 );
						VectorMA( saberBase2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].lengthMax, ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzleDir, saberTip2 );
						VectorMA( saberBaseNext2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].lengthMax, ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzleDirNext, saberTipNext2 );
					}
					*/
					if (checkDir)
					{//check the direction of the two swings to make sure the sabers are swinging towards each other
						...
						const float dot = DotProduct(ent1->client->saber[ent1saber_num].blade[ent1blade_num].muzzleDir,
							ent2->client->saber[ent2saber_num].blade[ent2blade_num].muzzleDir);
					...
					{
						G_TestLine(saberBase1, saberTip1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].color, 500);
						G_TestLine(saberTip1, saberTipNext1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].color, 500);
						G_TestLine(saberTipNext1, saberBase1, ent1->client->saber[ent1saber_num].blade[ent1blade_num].color, 500);

						G_TestLine(saberBase2, saberTip2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].color, 500);
						G_TestLine(saberTip2, saberTipNext2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].color, 500);
						G_TestLine(saberTipNext2, saberBase2, ent2->client->saber[ent2saber_num].blade[ent2blade_num].color, 500);
					}
#endif
...
static QINLINE int G_PowerLevelForSaberAnim(gentity_t* ent, int saber_num, qboolean mySaberHit)
{
	if (!ent || !ent->client || saber_num >= MAX_SABERS)
	{
		return FORCE_LEVEL_0;
	}
	const int anim = ent->client->ps.torsoAnim;
	const int	animTimer = ent->client->ps.torsoTimer;
	const int	animTimeElapsed = BG_AnimLength(ent->localAnimIndex, (animNumber_t)anim) - animTimer;
	const saberInfo_t* saber = &ent->client->saber[saber_num];
	...
	{//parries, knockaways and broken parries
		return FORCE_LEVEL_1;//FIXME: saber_anim_level?
	}
	switch (anim)
	{
		...
		/*
			if (pm->ps)
			{
				if ( ( pm->ps->legsAnimTimer >= 1450
						&& BG_AnimLength( g_entities[ps->client_num].client->clientInfo.animFileIndex, BOTH_JUMPATTACK6 ) - pm->ps->legsAnimTimer >= 400 )
					||(pm->ps->legsAnimTimer >= 400
						&& BG_AnimLength( g_entities[ps->client_num].client->clientInfo.animFileIndex, BOTH_JUMPATTACK6 ) - pm->ps->legsAnimTimer >= 1100 ) )
...
void WP_SaberDoHit(gentity_t* self, int saber_num, int blade_num)
{
	if (!numVictims)
	{
		return;
	...
	for (int i = 0; i < numVictims; i++)
	{
		...
		const gentity_t* victim = &g_entities[victimEntityNum[i]];
		...
		gentity_t* te = G_TempEntity(dmgSpot[i], EV_SABER_HIT);
		if (te)
		{
...
void WP_SaberRadiusDamage(gentity_t* ent, vec3_t point, float radius, int damage, float knock_back)
{
	...
	for (i = 0; i < num_ents; i++)
	{
		...
		{//in range
			if (damage > 0)
			{//do damage
				const int points = ceil((float)damage * dist / radius);
			...
			{//do knockback
				...
				{
					const float knockbackStr = knock_back * dist / radius;
					entDir[2] += 0.1f;
...
void WP_SaberDoClash(gentity_t* self, int saber_num, int blade_num)
{
...
void WP_SaberBounceSound(gentity_t* ent, int saber_num, int blade_num)
{
	if (!ent || !ent->client)
	{
		return;
...
static QINLINE qboolean CheckSaberDamage(gentity_t* self, int rsaber_num, int rblade_num, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate)
{
	static trace_t tr;
	static vec3_t dir;
	...
	saberBoxSize += (self->client->saber[rsaber_num].blade[rblade_num].radius * 0.5f);
	...
	else if (self->client->ps.fd.saber_anim_level < FORCE_LEVEL_2)
	...
	{ //just trace the minimum blade radius
		saberBoxSize = (self->client->saber[rsaber_num].blade[rblade_num].radius * 0.4f);
	...
	while (!saberTraceDone)
	{
		...
		{ //This didn't quite work out like I hoped. But it's better than nothing. Sort of.
			...
			{//trace from last pos
				VectorCopy(self->client->saber[rsaber_num].blade[rblade_num].trail.base, oldSaberStart);
				VectorCopy(self->client->saber[rsaber_num].blade[rblade_num].trail.tip, oldSaberEnd);
			}
	...
	if (self->client->ps.saberAttackWound < level.time
		&& (SaberAttacking(self)
			...
			|| (d_saberSPStyleDamage.integer && self->client->ps.saberInFlight && rsaber_num == 0)
			|| (WP_SaberBladeDoTransitionDamage(&self->client->saber[rsaber_num], rblade_num) && BG_SaberInTransitionAny(self->client->ps.saber_move))
			|| (self->client->ps.m_iVehicleNum && self->client->ps.saber_move > LS_READY))
		)
	{ //this animation is that of the last attack movement, and so it should do full damage
		qboolean saberInSpecial = BG_SaberInSpecial(self->client->ps.saber_move);
		qboolean inBackAttack = G_SaberInBackAttack(self->client->ps.saber_move);
		...
		if (d_saberSPStyleDamage.integer)
		{
			...
			{
				attackStr = G_PowerLevelForSaberAnim(self, rsaber_num, qfalse);
		...
		else
		{
				...
				self->client->ps.fd.saber_anim_level == SS_DUAL)
			{
				if (saberInSpecial)
				{
					...
					if (self->client->ps.saber_move == LS_SPINATTACK ||
						self->client->ps.saber_move == LS_SPINATTACK_DUAL)
					...
					else
					{
						...
						else if (BG_SaberInKata(self->client->ps.saber_move))
						{ //special kata move
							if (self->client->ps.fd.saber_anim_level == SS_DUAL)
			...
			else if (self->client->ps.fd.saber_anim_level == 3)
			{
			...
			else if (self->client->ps.fd.saber_anim_level == 2)
			{
				if (saberInSpecial &&
					(self->client->ps.saber_move == LS_A_FLIP_STAB || self->client->ps.saber_move == LS_A_FLIP_SLASH))
			...
			else if (self->client->ps.fd.saber_anim_level == 1)
			{
				if (saberInSpecial &&
					(self->client->ps.saber_move == LS_A_LUNGE))
				{
			...
			attackStr = self->client->ps.fd.saber_anim_level;
		}
	}
	...
	{ //just touching, do minimal damage and only check for it every 200ms (mainly to cut down on network traffic for hit events)
		...
		if (d_saberSPStyleDamage.integer)
		{
			if (BG_SaberInReturn(self->client->ps.saber_move))
			{
	...
	if (BG_SaberInSpecial(self->client->ps.saber_move))
	{
		...
		{
			if (self->client->ps.saber_move == LS_A_JUMP_T__B_)
			{ //do extra damage for special unblockables
				dmg += 5; //This is very tiny, because this move has a huge damage ramp
			}
			else if (self->client->ps.saber_move == LS_A_FLIP_STAB || self->client->ps.saber_move == LS_A_FLIP_SLASH)
			{
				dmg += 5; //ditto
			...
			else if (self->client->ps.saber_move == LS_A_LUNGE)
			{
			...
			else if (self->client->ps.saber_move == LS_SPINATTACK ||
				self->client->ps.saber_move == LS_SPINATTACK_DUAL)
	...
	if (dmg > SABER_NONATTACK_DAMAGE)
	{
		...
		if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rsaber_num], rblade_num)
			&& self->client->saber[rsaber_num].damageScale != 1.0f)
		{
			dmg = ceil((float)dmg * self->client->saber[rsaber_num].damageScale);
		}
		else if (WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rsaber_num], rblade_num)
			&& self->client->saber[rsaber_num].damageScale2 != 1.0f)
		{
			dmg = ceil((float)dmg * self->client->saber[rsaber_num].damageScale2);
		}
	...
	if (saberHitWall
		&& (self->client->saber[rsaber_num].saberFlags & SFL_BOUNCE_ON_WALLS)
		&& (BG_SaberInAttackPure(self->client->ps.saber_move) //only in a normal attack anim
			|| self->client->ps.saber_move == LS_A_JUMP_T__B_) //or in the strong jump-fwd-attack "death from above" move
		)
	{ //then bounce off
		/*
		...
		if (!skipIt && (!onlyIfNotSpecial || !BG_SaberInSpecial(self->client->ps.saber_move)))
		*/
		{
			gentity_t* te = NULL;
			/*
			...
			WP_SaberDoClash( self, rsaber_num, rblade_num );
			saberDoClashEffect = pre;
			*/

			self->client->ps.saber_move = BG_BrokenParryForAttack(self->client->ps.saber_move);
			...
			{ //set anim now on both parts
				int anim = saberMoveData[self->client->ps.saber_move].animToUse;
			...
			WP_SaberBounceSound(self, rsaber_num, rblade_num);
			//do hit effect
			...
			te->s.weapon = rsaber_num;
			te->s.legsAnim = rblade_num;
			...
			if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rsaber_num], rblade_num))
			{
				WP_SaberRadiusDamage(self, tr.endpos, self->client->saber[rsaber_num].splashRadius, self->client->saber[rsaber_num].splashDamage, self->client->saber[rsaber_num].splashKnockback);
			}
			else
			{
				WP_SaberRadiusDamage(self, tr.endpos, self->client->saber[rsaber_num].splashRadius2, self->client->saber[rsaber_num].splashDamage2, self->client->saber[rsaber_num].splashKnockback2);
			}
	...
	{//hit something that had health and takes damage
		...
		if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rsaber_num], rblade_num)
			&& !(self->client->saber[rsaber_num].saberFlags2 & SFL2_NO_DISMEMBERMENT))
		{
		...
		if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rsaber_num], rblade_num)
			&& self->client->saber[rsaber_num].knockbackScale > 0.0f)
		{
			if (rsaber_num < 1)
			{
	...
	{ //saber clash
		...
		{//hit an in-hand saber, do extra collision check against it
			...
			{//use SP-style blade-collision test
				if (!WP_SabersIntersect(self, rsaber_num, rblade_num, otherOwner, qfalse))
	...
	blockStuff:
		...
		if (BG_SaberInSpecial(otherOwner->client->ps.saber_move))
		{
		...
		if (((selfSaberLevel < FORCE_LEVEL_3 && ((tryDeflectAgain && Q_irand(1, 10) <= 3) || (!tryDeflectAgain && Q_irand(1, 10) <= 7))) || (Q_irand(1, 10) <= 1 && otherSaberLevel >= FORCE_LEVEL_3))
			...
			&& !PM_SaberInBrokenParry(otherOwner->client->ps.saber_move)
			&& !BG_SaberInSpecial(otherOwner->client->ps.saber_move)
			&& !PM_SaberInBounce(otherOwner->client->ps.saber_move)
			&& !PM_SaberInDeflect(otherOwner->client->ps.saber_move)
			&& !PM_SaberInReflect(otherOwner->client->ps.saber_move)
		...
		{//knockaways can make fast-attacker go into a broken parry anim if the ent is using fast or med. In MP, we also randomly decide this for level 3 attacks.
			...
			if (!PM_SaberInParry(otherOwner->client->ps.saber_move))
			{
				WP_SaberBlockNonRandom(otherOwner, tr.endpos, qfalse);
				otherOwner->client->ps.saber_move = BG_KnockawayForParry(otherOwner->client->ps.saberBlocked);
				otherOwner->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
			}
			else
			{
				otherOwner->client->ps.saber_move = G_KnockawayForParry(otherOwner->client->ps.saber_move); //BG_KnockawayForParry( otherOwner->client->ps.saberBlocked );
				otherOwner->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
			}
			...
			PM_SaberInParry(otherOwner->client->ps.saber_move) &&
			!PM_SaberInBrokenParry(otherOwner->client->ps.saber_move) &&
			!PM_SaberInParry(self->client->ps.saber_move) &&
			!PM_SaberInBrokenParry(self->client->ps.saber_move) &&
			!PM_SaberInBounce(self->client->ps.saber_move) &&
		...
		{ //they are in a parry, and we are slamming down on them with a move of equal or greater force than their defense, so send them into a broken parry.. unless they are already in one.
			...
			otherOwner->client->ps.saber_move = BG_BrokenParryForParry(otherOwner->client->ps.saber_move);
		...
		else if (SaberAttacking(otherOwner) && dmg > SABER_NONATTACK_DAMAGE && !BG_SaberInSpecial(otherOwner->client->ps.saber_move) && !didOffense && !otherUnblockable)
		{ //they were attacking and our saber hit their saber, make them bounce. But if they're in a special attack, leave them.
				...
				!PM_SaberInReflect(otherOwner->client->ps.saber_move))
			{
				...
				{//I won, he should knockaway
					otherOwner->client->ps.saber_move = BG_BrokenParryForAttack(otherOwner->client->ps.saber_move);
					otherOwner->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
				}
				else if (attackAdv > 0)
		...
		{ //block
				...
				!PM_SaberInReflect(otherOwner->client->ps.saber_move))
			{
				...
				{
					int otherIdleStr = otherOwner->client->ps.fd.saber_anim_level;
				...
				{ //This means that the attack actually hit our saber, and we went to block it.
				  //But, one of the above cases says we actually can't. So we will be smashed into a broken parry instead.
					otherOwner->client->ps.saber_move = BG_BrokenParryForParry(G_GetParryForBlock(otherOwner->client->ps.saberBlocked));
					otherOwner->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
				...
				{ //We want to try deflecting again because the other is in the parry and we haven't made any new moves
					int preMove = otherOwner->client->ps.saber_move;

					otherOwner->client->ps.saber_move = G_GetParryForBlock(otherOwner->client->ps.saberBlocked);
					WP_GetSaberDeflectionAngle(self, otherOwner, tr.fraction);
					otherOwner->client->ps.saber_move = preMove;
				}
			}
		}
...
void G_SPSaberDamageTraceLerped(gentity_t* self, int saber_num, int blade_num, vec3_t baseNew, vec3_t endNew, int clipmask)
{
	vec3_t baseOld, endOld;
	vec3_t mp1, mp2;
	vec3_t md1, md2;

	if ((level.time - self->client->saber[saber_num].blade[blade_num].trail.lastTime) > 100)
	{//no valid last pos, use current
	...
	{//trace from last pos
		VectorCopy(self->client->saber[saber_num].blade[blade_num].trail.base, baseOld);
		VectorCopy(self->client->saber[saber_num].blade[blade_num].trail.tip, endOld);
	}
	...
	{//no diff
		CheckSaberDamage(self, saber_num, blade_num, baseNew, endNew, qfalse, clipmask, qfalse);
	}
	else
	{//saber moved, lerp
		const float stepsize = 8;//aveLength,
		...
		CheckSaberDamage(self, saber_num, blade_num, bladePointOld, bladePointNew, qfalse, clipmask, qtrue);
		...
		if (saberHitFraction < 1.0f)
		{
			...
			VectorMA(baseNew, self->client->saber[saber_num].blade[blade_num].lengthMax, md2, endNew);
		}
		...
		if (BG_SaberInAttack(self->client->ps.saber_move)
			...
			//|| (g_timescale->value<1.0f&&BG_SaberInTransitionAny( ent->client->ps.saber_move )) )
		{
		...
		while (1)
		{
			...
			for (float step = stepsize; step <= self->client->saber[saber_num].blade[blade_num].lengthMax /*&& step < self->client->saber[saber_num].blade[blade_num].lengthOld*/; step += stepsize)
			{
				VectorMA(curBase1, step, curMD1, bladePointOld);
				VectorMA(curBase2, step, curMD2, bladePointNew);

				if (step + stepsize >= self->client->saber[saber_num].blade[blade_num].lengthMax)
				{
					extrapolate = qfalse;
				}
				//do the damage trace
				CheckSaberDamage(self, saber_num, blade_num, bladePointOld, bladePointNew, qfalse, clipmask, extrapolate);
				/*
				if ( WP_SaberDamageForTrace( ent->s.number, bladePointOld, bladePointNew, baseDamage, curMD2,
					qfalse, entPowerLevel, ent->client->ps.saber[saber_num].type, qtrue,
					saber_num, blade_num ) )
				{
					hit_wall = qtrue;
				}
				*/
				...
				if (saberHitFraction < 1.0f)
				{
					...
					VectorSubtract(mp2, mp1, baseDiff);
					VectorMA(mp1, saberHitFraction, baseDiff, baseNew);
					VectorMA(baseNew, self->client->saber[saber_num].blade[blade_num].lengthMax, curMD2, endNew);
					//adjust muzzleDir...
		...
		/*
		aveLength = (ent->client->ps.saber[saber_num].blade[blade_num].lengthOld + ent->client->ps.saber[saber_num].blade[blade_num].length)/2;
		if ( step > aveLength )
		...
		if ( WP_SaberDamageForTrace( ent->s.number, endOld, endNew, tipDmgMod*baseDamage, md2,
			qfalse, entPowerLevel, ent->client->ps.saber[saber_num].type, qfalse,
			saber_num, blade_num ) )
		{
			hit_wall = qtrue;
		}
...
void wp_saber_start_missile_block_check(gentity_t* self, usercmd_t* ucmd)
{
	...
	{//don't do this if already attacking!
		if (ucmd->buttons & BUTTON_ATTACK
			|| BG_SaberInAttack(self->client->ps.saber_move)
			|| BG_SaberInSpecialAttack(self->client->ps.torsoAnim)
			|| BG_SaberInTransitionAny(self->client->ps.saber_move))
		{
			doFullRoutine = qfalse;
		}
	}
	...
	for (int e = 0; e < num_listed_entities; e++)
	{
		...
		{ //seems like a valid enemy to look at.
			...
			const float vecLen = VectorLength(vecSub);
		...
		{//not a normal projectile
			...
			const gentity_t* pOwner = &g_entities[ent->r.ownerNum];
...
static QINLINE void saberMoveBack(gentity_t* ent, qboolean goingBack)
{
	...
	{ //acts as a fallback in case touch code fails, keeps saber from going through things between predictions
		const int iCompensationLength = 32;
		trace_t tr;
		vec3_t mins, maxs;
		...
		const float originalLength = VectorLength(calcComp);
...
qboolean saberCheckKnockdown_DuelLoss(gentity_t* saberent, gentity_t* saberOwner, gentity_t* other)
{
	...
	saberOwner->client->ps.saber_move = LS_V1_BL; //rwwFIXMEFIXME: Ideally check which lock it was exactly and use the proper anim (same goes for the attacker)
...
qboolean saberCheckKnockdown_BrokenParry(gentity_t* saberent, gentity_t* saberOwner, gentity_t* other)
{
	...
	//saber out of hand.
	const int myAttack = G_SaberAttackPower(saberOwner, qfalse);
	const int otherAttack = G_SaberAttackPower(other, qfalse);
...
void saberBackToOwner(gentity_t* saberent)
{
	const gentity_t* saberOwner = &g_entities[saberent->r.ownerNum];
	vec3_t dir;
...
{
	vec3_t		vSub;
	const gentity_t* saberOwn = &g_entities[saberent->r.ownerNum];
...
static void G_KickSomeMofos(gentity_t* ent)
{
	...
	const float animLength = BG_AnimLength(ent->localAnimIndex, (animNumber_t)ent->client->ps.legsAnim);
	const float elapsedTime = (float)(animLength - ent->client->ps.legsTimer);
	const float remainingTime = (animLength - elapsedTime);
	float kickDist = (ent->r.maxs[0] * 1.5f) + STAFF_KICK_RANGE + 8.0f;//fudge factor of 8
	const int	  kickDamage = Q_irand(10, 15);//Q_irand( 3, 8 ); //since it can only hit a guy once now
...
nextStep:
	...
	if (returnAfterUpdate && saber_num)
	...
	if (!BG_SabersOff(&self->client->ps))
	{
		...
		if (self->client->ps.saberLockTime > level.time && self->client->ps.saberEntityNum)
		{
			if (self->client->ps.saberIdleWound < level.time)
			{
				gentity_t* te;
				vec3_t dir;
				te = G_TempEntity(g_entities[saber_num].r.currentOrigin, EV_SABER_BLOCK);
				VectorSet(dir, 0, 1, 0);
				VectorCopy(g_entities[saber_num].r.currentOrigin, te->s.origin);
			...
			while (rsaber_num < MAX_SABERS)
			{
				rblade_num = 0;
				while (rblade_num < self->client->saber[rsaber_num].numBlades)
				{ //Don't bother updating the bolt for each blade for this, it's just a very rough fallback method for during saberlocks
					VectorCopy(boltOrigin, self->client->saber[rsaber_num].blade[rblade_num].trail.base);
					VectorCopy(end, self->client->saber[rsaber_num].blade[rblade_num].trail.tip);
					self->client->saber[rsaber_num].blade[rblade_num].trail.lastTime = level.time;
		...
		while (rsaber_num < MAX_SABERS)
		{
			if (!self->client->saber[rsaber_num].model[0])
			{
				rsaber_num++;
				continue;
			}
			...
			/*
			if (rsaber_num == 0 && (self->client->ps.brokenLimbs & (1 << BROKENLIMB_RARM)))
			...
			*/
			...
			if (rsaber_num == 1 && (self->client->ps.brokenLimbs & (1 << BROKENLIMB_LARM)))
			...
			while (rblade_num < self->client->saber[rsaber_num].numBlades)
			{
				...
				VectorCopy(self->client->saber[rsaber_num].blade[rblade_num].muzzlePoint, self->client->saber[rsaber_num].blade[rblade_num].muzzlePointOld);
				VectorCopy(self->client->saber[rsaber_num].blade[rblade_num].muzzleDir, self->client->saber[rsaber_num].blade[rblade_num].muzzleDirOld);

				if (rblade_num > 0 //more than one blade
					&& (!self->client->saber[1].model[0])//not using dual blades
					&& self->client->saber[rsaber_num].numBlades > 1//using a multi-bladed saber
					&& self->client->ps.saberHolstered == 1)//
				...
				if (rsaber_num == 0 && self->client->ps.saberInFlight)
				{
					if (!self->client->ps.saberEntityNum)
				...
				self->client->saber[rsaber_num].blade[rblade_num].storageTime = level.time;
				...
				if (self->client->hasCurrentPosition && d_saberInterpolate.integer)
				{
					...
					{ //rww - 07/17/02 - don't bother doing the extra stuff unless actually attacking. This is in attempt to save CPU.
						CheckSaberDamage(self, rsaber_num, rblade_num, boltOrigin, end, qfalse, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT), qfalse);
					}
					else if (d_saberInterpolate.integer == 1)
					{
						...
						while (!gotHit)
						{
							if (!CheckSaberDamage(self, rsaber_num, rblade_num, boltOrigin, end, qfalse, trMask, qfalse))
							{
								if (!CheckSaberDamage(self, rsaber_num, rblade_num, boltOrigin, end, qtrue, trMask, qfalse))
								{
									...
									if ((deltaX != 0 || deltaY != 0 || deltaZ != 0) && deltaX < 180 && deltaY < 180 && deltaZ < 180 && (BG_SaberInAttack(self->client->ps.saber_move) || PM_SaberInTransition(self->client->ps.saber_move)))
									...
									//and don't bother if the angle is the same
										...
										saberMidEnd[0] = saberMidPoint[0] + saberMidDir[0] * self->client->saber[rsaber_num].blade[rblade_num].lengthMax;
										saberMidEnd[1] = saberMidPoint[1] + saberMidDir[1] * self->client->saber[rsaber_num].blade[rblade_num].lengthMax;
										saberMidEnd[2] = saberMidPoint[2] + saberMidDir[2] * self->client->saber[rsaber_num].blade[rblade_num].lengthMax;

										//I'll just trace straight out and not even trace between positions to save speed.
										if (CheckSaberDamage(self, rsaber_num, rblade_num, saberMidPoint, saberMidEnd, qfalse, trMask, qfalse))
										{
											gotHit = qtrue;
										}
									}
					...
					{
						if (!CheckSaberDamage(self, rsaber_num, rblade_num, boltOrigin, end, qfalse, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT), qfalse))
						{
							CheckSaberDamage(self, rsaber_num, rblade_num, boltOrigin, end, qtrue, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT), qfalse);
						}
					}
				}
				else if (d_saberSPStyleDamage.integer)
				{
					G_SPSaberDamageTraceLerped(self, rsaber_num, rblade_num, boltOrigin, end, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT));
				}
				else
				{
					CheckSaberDamage(self, rsaber_num, rblade_num, boltOrigin, end, qfalse, (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT), qfalse);
				...
				//do hit effects
				WP_SaberDoHit(self, rsaber_num, rblade_num);
				WP_SaberDoClash(self, rsaber_num, rblade_num);
...
void WP_SaberBlock(gentity_t* playerent, vec3_t hitloc, qboolean missileBlock)
{
	...
	const float rightdot = DotProduct(right, diff) + RandFloat(-0.2f, 0.2f);
	const float zdiff = hitloc[2] - playerent->client->ps.origin[2] + Q_irand(-8, 8);
