...
void CG_SurfaceExplosion(vec3_t origin, vec3_t normal, float radius, float shake_speed, qboolean smoke)
{
	...
	//Sparks
	const int numSparks = 16 + (Q_flrand(0.0f, 1.0f) * 16.0f);
	...
	localEntity_t* le = CG_MakeExplosion(origin, direction, cgs.media.explosionModel, 6,
		cgs.media.surfaceExplosionShader, 500, qfalse,
		radius * 0.02f + (Q_flrand(0.0f, 1.0f) * 0.3f), 0);
	le->light = 150;
