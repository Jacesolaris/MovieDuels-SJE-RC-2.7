...
sfxHandle_t	CG_CustomSound(int client_num, const char* soundName) {
	...
	for (i = 0; i < MAX_CUSTOM_SOUNDS; i++)
	{
		...
		if ((cgs.gametype >= GT_TEAM || com_buildScript.integer) && i < numCSiegeSounds && !strcmp(lSoundName, bg_customSiegeSoundNames[i]))
		{ //siege only
			return ci->siegeSounds[i];
		}
		if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL || com_buildScript.integer) && i < numCDuelSounds && !strcmp(lSoundName, cg_customDuelSoundNames[i]))
		{ //siege only
	...
	*ci = newInfo;
	...
	{	// Copy the new ghoul2 model to the centity.
		...
		if (anim)
		{
			...
			trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", firstFrame, anim->firstFrame + anim->num_frames, flags, anim_speed, cg.time, setFrame, 150);
		...
		if (anim)
		{
			...
			//rww - Set the animation again because it just got reset due to the model change
			trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "lower_lumbar", firstFrame, anim->firstFrame + anim->num_frames, flags, anim_speed, cg.time, setFrame, 150);
...
void CG_PlayerAnimEventDo(centity_t* cent, animevent_t* anim_event)
{
	...
	const clientInfo_t* client = NULL;
	...
	{	// are there variations on the sound?
		const int holdSnd = anim_event->eventData[AED_SOUNDINDEX_START + Q_irand(0, anim_event->eventData[AED_SOUND_NUMRANDOMSNDS])];
		if (holdSnd > 0)
		{
	...
	case AEV_SABER_SPIN:
		...
		break;
	case AEV_FOOTSTEP:
		CG_PlayerFootsteps(cent, (footstepType_t)anim_event->eventData[AED_FOOTSTEP_TYPE]);
		break;
	case AEV_EFFECT:
#if 0 //SP method
		...
		if (anim_event->eventData[AED_BOLTINDEX] != -1)
		{
			...
			trap->G2API_GetBoltMatrix(cent->ghoul2, anim_event->eventData[AED_MODELINDEX], anim_event->eventData[AED_BOLTINDEX], &matrix, lAngles,
			...
			trap->FX_PlayEffectID(anim_event->eventData[AED_EFFECTINDEX], bPoint, bAngle, -1, -1, qfalse);
		}
		else
		{
			vec3_t bAngle;
...
{ //this also sets the lerp frames, so I suggest you keep calling it regardless of if you want anim sounds.
	...
	if (trap->G2API_GetBoneFrame(cent->ghoul2, "model_root", cg.time, &current_frame, cgs.gameModels, 0))
	{
		// the above may have failed, not sure what to do about it, current frame will be zero in that case
		curFrame = floor(current_frame);
	}
	...
	if (trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time, &current_frame, cgs.gameModels, 0))
	{
		curFrame = floor(current_frame);
	}
	...
	cent->pe.torso.backlerp = 1.0f - (current_frame - (float)curFrame);
}
...
static void CG_SetLerpFrameAnimation(centity_t* cent, clientInfo_t* ci, lerpFrame_t* lf, int newAnimation, float animSpeedMult, qboolean torsoOnly, qboolean flipState) {
	...
	if (cent->ghoul2)
	{
		...
		if (anim_speed < 0)
		{
			...
			firstFrame = anim->firstFrame + anim->num_frames;
		}
		else
		{
			firstFrame = anim->firstFrame;
			lastFrame = anim->firstFrame + anim->num_frames;
		}
		...
		anim_speed *= animSpeedMult;

		BG_SaberStartTransAnim(cent->currentState.number, cent->currentState.fireflag, cent->currentState.weapon, newAnimation, &anim_speed, cent->currentState.brokenLimbs);
		...
		{
			trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", firstFrame, lastFrame, flags, anim_speed, cg.time, beginFrame, blend_time);
			return;
		}
		...
		{ //rww - The guesswork based on the lerp frame figures is usually BS, so I've resorted to a call to get the frame of the bone directly.
			...
			if ((cent->currentState.torsoAnim) == (cent->currentState.legsAnim) && GBAcFrame >= anim->firstFrame && GBAcFrame <= (anim->firstFrame + anim->num_frames))
			...
			trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", firstFrame, lastFrame, flags, anim_speed, cg.time, beginFrame, blend_time);
		...
		else
		{
			...
			if (cent->currentState.torsoAnim == cent->currentState.legsAnim &&
				(ci->legsAnim != newAnimation || oldSpeed != anim_speed))
		...
		{ //make sure we're humanoid before we access the motion bone
			trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", firstFrame, lastFrame, flags, anim_speed, cg.time, beginFrame, blend_time);
		}
...
static void CG_RunLerpFrame(centity_t* cent, clientInfo_t* ci, lerpFrame_t* lf, qboolean flipState, int newAnimation, float speedScale, qboolean torsoOnly)
{
	...
	if (cent->currentState.forceFrame)
	{
		if (lf->lastForcedFrame != cent->currentState.forceFrame)
		{
			...
			trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, anim_speed, cg.time, -1, 150);
			trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, anim_speed, cg.time, -1, 150);
			trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, anim_speed, cg.time, -1, 150);
		}
...
static void CG_PlayerAnimation(centity_t* cent, int* legs_old, int* legs, float* legs_back_lerp,
	int* torso_old, int* torso, float* torso_back_lerp) {
	clientInfo_t* ci;
	float			speedScale;
...
qboolean CG_RagDoll(centity_t* cent, vec3_t forcedAngles)
{
	...
	{ //If we're not in a ragdoll state, perform the checks.
		...
		if (!inSomething)
		{
			...
			const int dur = (bgAllAnims[cent->localAnimIndex].anims[anim].num_frames - 1) * fabs((float)(bgAllAnims[cent->localAnimIndex].anims[anim].frameLerp));
			int i = 0;
			int boltChecks[5];
			vec3_t boltPoints[5];
	...
	{ //We're in a ragdoll state, so make the call to keep our positions updated and whatnot.
		...
		int ragAnim = CG_RagAnimForPositioning(cent);
		...
		tParms.start_frame = bgAllAnims[cent->localAnimIndex].anims[ragAnim].firstFrame;// + bgAllAnims[cent->localAnimIndex].anims[ragAnim].num_frames;
		tParms.end_frame = bgAllAnims[cent->localAnimIndex].anims[ragAnim].firstFrame + bgAllAnims[cent->localAnimIndex].anims[ragAnim].num_frames;
#if 0
		{
			float anim_speed = 0;
			int blend_time = 600;
...
#elif 1 //with my new method of doing things I want it to continue the anim
		{
			...
			if (trap->G2API_GetBoneAnim(cent->ghoul2, "model_root", cg.time, &current_frame, &start_frame, &end_frame, &flags, &anim_speed, cgs.gameModels, 0))
			{ //lock the anim on the current frame.
				int blend_time = 500;
				const animation_t* curAnim = &bgAllAnims[cent->localAnimIndex].anims[cent->currentState.legsAnim];

				if (current_frame >= (curAnim->firstFrame + curAnim->num_frames - 1))
				{ //this is sort of silly but it works for now.
					current_frame = (curAnim->firstFrame + curAnim->num_frames - 2);
				}

				trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", current_frame, current_frame + 1, flags, anim_speed, cg.time, current_frame, blend_time);
				trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", current_frame, current_frame + 1, flags, anim_speed, cg.time, current_frame, blend_time);
				trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", current_frame, current_frame + 1, flags, anim_speed, cg.time, current_frame, blend_time);
			}
		}
#endif
			...
			(cent->currentState.eType != ET_NPC || cent->currentState.NPC_class != CLASS_VEHICLE))
		{
			...
			if (grabEnt->ghoul2)
			{
				...
				const int thorBolt = trap->G2API_AddBolt(cent->ghoul2, 0, "thoracic");
	...
	{ //cycle through the 4 bone index values on the entstate
		...
		{ //if it's non-0 then it could have something in it.
			...
			{ //got the bone, now set the angles from the corresponding entitystate boneangles value.
				...
				trap->G2API_SetBoneAngles(cent->ghoul2, 0, bone_name, boneAngles, flags, up, right, forward, cgs.gameModels, 100, cg.time);
			}
		}
...
//	float		timeScaleMod = (cg_timescale.value&&gent&&gent->s.client_num==0&&!player_locked&&!MatrixMode&&gent->client->ps.forcePowersActive&(1<<FP_SPEED))?(1.0/cg_timescale.value):1.0;
	...
	const float anim_speed = 50.0f / animations[anim].frameLerp * timeScaleMod;
	int	firstFrame;
	int	lastFrame;
	...
	{//play anim backwards
		...
		firstFrame = (animations[anim].num_frames - 1) + animations[anim].firstFrame;
	}
	else
	{
		firstFrame = animations[anim].firstFrame;
		lastFrame = (animations[anim].num_frames) + animations[anim].firstFrame;
	}
...
//	if (!animatingHead || ( animations[anim].firstFrame != start_frame ) )// only set the anim if we aren't going to do the same animation again
	{
		...
		trap->G2API_SetBoneAnim(cent->ghoul2, 0, "face", firstFrame, lastFrame, animFlags, anim_speed,
...
void CG_DrawPlayerShield(centity_t* cent, vec3_t origin)
{
	...
	int alpha = 255.0 * ((cent->damageTime - cg.time) / MIN_SHIELD_TIME) + Q_flrand(0.0f, 1.0f) * 16;
	if (alpha > 255)
		alpha = 255;
...
qboolean CG_G2TraceCollide(trace_t* tr, vec3_t const mins, vec3_t const maxs, const vec3_t lastValidStart, const vec3_t lastValidEnd)
{
	...
	centity_t* g2Hit = &cg_entities[tr->entity_num];
	...
	while (!doneWithTraces)
	{
		...
		{ //hit a client..
			...
			{ //it succeeded with the ghoul2 trace
				...
				trap->S_StartSound(trace.endpos, trace.entity_num, CHAN_AUTO, trap->S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3))));
			}
		}
		...
		{ //this is the guy the server says we last hit, so continue.
			...
			{ //If it has a g2 instance, do the proper ghoul2 checks
				...
				{ //it succeeded with the ghoul2 trace
					...
					{
						vec3_t ePos;
						centity_t* trEnt = &cg_entities[trace.entity_num];
						...
						if (trEnt->ghoul2)
						{
							...
							{ //don't do on fighters cause they have crazy full axial angles
								...
								CG_AddGhoul2Mark(markShader, flrand(3.0f, 4.0f),
									trace.endpos, ePos, trace.entity_num, trEnt->lerpOrigin, trEnt->lerpAngles[YAW],
		...
		for (i = 0; i < 1; i++)//was 2 because it would go through architecture and leave saber trails on either side of the brush - but still looks bad if we hit a corner, blade is still 8 longer than hit
		{
			...
			if (trace.fraction < 1.0f)
			{
				...
				{//draw marks if we hit a wall
					...
					{
						if (trace.entity_num == ENTITYNUM_WORLD || cg_entities[trace.entity_num].currentState.eType == ET_TERRAIN || (cg_entities[trace.entity_num].currentState.eFlags & EF_PERMANENT))
...
CheckTrail:
	...
	{
		saberTrail->duration = saberMoveData[cent->currentState.saber_move].trailLength;
		...
		if (cg.time > saberTrail->lastTime + 2 || cg_saberTrail.integer == 2) // 2ms
		{
			if (!dontDraw)
			{
				if ((BG_SuperBreakWinAnim(cent->currentState.torsoAnim) || saberMoveData[cent->currentState.saber_move].trailLength > 0 || ((cent->currentState.powerups & (1 << PW_SPEED) && cg_speedTrail.integer)) || (cent->currentState.saberInFlight && saber_num == 0)) && cg.time < saberTrail->lastTime + 2000) // if we have a stale segment, don't draw until we have a fresh one
				{
#if 0
					if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4)
					{
...
#endif
					{
						...
						{ //don't draw it if the last time is way out of date
							...
							if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4)
							{
								...
								if (BG_SaberInAttack(cent->currentState.saber_move)
...
void CG_AddLightningBeam(vec3_t start, vec3_t end)
{
	...
	const float s1 = sin(cg.time * 0.005f) * 2 + Q_flrand(-1.0f, 1.0f) * 0.2f;
	...
	if (cModelName && cModelName[0])
	{
		...
		if (cent->ghoul2)
		{
			...
			{ //do special vehicle stuff
				...
				int i = trap->G2API_AddBolt(cent->ghoul2, 0, "model_root");
...
void CG_Player(centity_t* cent) {
		...
		&& cent->currentState.NPC_class != CLASS_VEHICLE)
	{
		...
		{//arc
			...
			//trap->FX_PlayEntityEffectID(cgs.effects.forceDrainWide, efOrg, axis, cent->bolt_info, cent->currentState.number, -1, -1);
		...
		{//line
			...
			//trap->FX_PlayEntityEffectID(cgs.effects.forceDrain, efOrg, axis, cent->bolt_info, cent->currentState.number, -1, -1);
	...
	{//doing the electrocuting
		...
		{//arc
			//trap->FX_PlayEffectID( cgs.effects.forceLightningWide, efOrg, fxDir );
			//trap->FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrg, axis, cent->bolt_info, cent->currentState.number, -1, -1);
			trap->FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrg, axis, -1, -1, -1, -1);
		}
		else
		{//line
			//trap->FX_PlayEffectID( cgs.effects.forceLightning, efOrg, fxDir );
			//trap->FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrg, axis, cent->bolt_info, cent->currentState.number, -1, -1);
		...
		cent->currentState.number))
	{
		if (cent->ghoul2)
		{
			...
			//trap->FX_PlayEntityEffectID(trap->FX_RegisterEffect("force/confusion.efx"), efOrg, axis, cent->bolt_info, cent->currentState.number);
