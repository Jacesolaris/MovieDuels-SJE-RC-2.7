...
void CG_S_UpdateLoopingSounds(int entity_num)
{
	...
	while (i < cent->numLoopingSounds)
	{
		...
		trap->S_AddLoopingSound(entity_num, lerpOrg, cSound->velocity, cSound->sfx);
		i++;
	}
}
	...
	{//I'm the hyperspace brush
			...
			&& (cg.time - cg.predictedVehicleState.hyperSpaceTime) > 1000)
		{
			...
			{//actually hyperspacing now
				const float timeFrac = ((float)(cg.time - cg.predictedVehicleState.hyperSpaceTime - 1000)) / (HYPERSPACE_TIME - 1000);
