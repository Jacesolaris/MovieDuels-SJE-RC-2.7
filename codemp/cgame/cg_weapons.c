	...
	*/
	...
	if ((ps || cg.renderingThirdPerson || cg.predictedPlayerState.client_num != cent->currentState.number) &&
		((cent->currentState.modelindex2 == WEAPON_CHARGING_ALT && cent->currentState.weapon == WP_BRYAR_PISTOL) ||
			...
			(cent->currentState.modelindex2 == WEAPON_CHARGING_ALT && cent->currentState.weapon == WP_CLONEPISTOL)))
	{
		int		shader = 0;
		float	val = 0.0f;
		float	scale = 1.0f;
		...
		else
		{
			...
			if (!(trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &bolt_matrix, newAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale)))
...
void CG_AddViewWeapon(playerState_t* ps) {
	...
	{
		const float fracDistFOV = tanf(cg.refdef.fov_x * (M_PI / 180) * 0.5f);
		const float fracWeapFOV = (1.0f / fracDistFOV) * tanf(cgFov * (M_PI / 180) * 0.5f);
...
void CG_GetClientWeaponMuzzleBoltPoint(int clIndex, vec3_t to)
{
	...
	trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &bolt_matrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
