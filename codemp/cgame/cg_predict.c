...
static void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
	...
	for (i = 0; i < cg_numSolidEntities; i++) {
		...
		if (ent->number > MAX_CLIENTS &&
			(ent->genericenemyindex - MAX_GENTITIES == cg.predictedPlayerState.client_num || ent->genericenemyindex - MAX_GENTITIES == cg.predictedVehicleState.client_num))
			//		if (ent->number > MAX_CLIENTS && cg.snap && ent->genericenemyindex && (ent->genericenemyindex-MAX_GENTITIES) == cg.snap->ps.client_num)
