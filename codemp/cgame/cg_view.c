...
static void CG_OffsetThirdPersonView( void ) {
	...
	if (!cg_cameraMode.integer) {
		CG_Trace( &trace, cg.refdef.vieworg, mins, maxs, view, cg.predictedPlayerState.client_num, MASK_CAMERACLIP);
...
void CG_SE_UpdateShake(vec3_t origin, vec3_t angles)
{
	...
	const float intensity_scale = 1.0f - ((float)(cg.time - cgScreenEffects.shake_start) / (float)cgScreenEffects.
		shake_duration) * (((cgScreenEffects.FOV + cgScreenEffects.FOV2) / 2.0f) / 90.0f);

	const float intensity = cgScreenEffects.shake_intensity * intensity_scale;
