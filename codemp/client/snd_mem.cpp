...
void R_CheckMP3s(const char* psDir)
{
	...
	for (i = 0; i < numSysFiles; i++)
	{
		...
		//
		byte* pbData = NULL;
		const int iSize = FS_ReadFile(sFilename, (void**)&pbData);
	...
	if (Q_stricmpn(psExt, ".mp3", 4) == 0)
	{
		...
		if (MP3_IsValid(sLoadName, data, size, qfalse))
		{
			...
			else
			{
				...
				//
				{
					byte* pbUnpackBuffer = static_cast<byte*>(Z_Malloc(iRawPCMDataSize + 10 + 2304 /* <g> */, TAG_TEMP_WORKSPACE, qfalse));	// won't return if fails
