...
static void CGFX_AddPoly(addpolyArgStruct_t* p) {
	FX_AddPoly(p->p, p->ev, p->num_verts, p->vel, p->accel, p->alpha1, p->alpha2, p->alphaParm, p->rgb1, p->rgb2, p->rgbParm, p->rotationDelta, p->bounce, p->motionDelay, p->killTime, p->shader, p->flags);
}
...
static qboolean CGFX_PlayBoltedEffectID(int id, vec3_t org, void* ghoul2, const int boltNum, const int ent_num, const int modelNum, int iLooptime, qboolean is_relative) {
	...
	{
		FX_PlayBoltedEffectID(id, org, bolt_info, &g2, iLooptime, is_relative);
		return qtrue;
	}
	return qfalse;
}
...
static void CL_G2API_ListModelBones(void* ghl_info, int frame) {
...
static qboolean CL_G2API_GetBoltMatrix(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* model_list, vec3_t scale) {
	if (!ghoul2) return qfalse;
	return re->G2API_GetBoltMatrix(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bolt_index, matrix, angles, position, frameNum, model_list, scale);
}

static qboolean CL_G2API_GetBoltMatrix_NoReconstruct(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* model_list, vec3_t scale) {
	if (!ghoul2) return qfalse;
	re->G2API_BoltMatrixReconstruction(qfalse);
	return re->G2API_GetBoltMatrix(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bolt_index, matrix, angles, position, frameNum, model_list, scale);
}

static qboolean CL_G2API_GetBoltMatrix_NoRecNoRot(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* model_list, vec3_t scale) {
	if (!ghoul2) return qfalse;
...
static void CL_G2API_CollisionDetect(CollisionRecord_t* collRecMap, void* ghoul2, const vec3_t angles, const vec3_t position, int frameNumber, int ent_num, vec3_t rayStart, vec3_t rayEnd, vec3_t scale, int traceFlags, int use_lod, float fRadius) {
	if (!ghoul2) return;
	re->G2API_CollisionDetect(collRecMap, *static_cast<CGhoul2Info_v*>(ghoul2), angles, position, frameNumber, ent_num, rayStart, rayEnd, scale, G2VertSpaceClient, traceFlags, use_lod, fRadius);
}

static void CL_G2API_CollisionDetectCache(CollisionRecord_t* collRecMap, void* ghoul2, const vec3_t angles, const vec3_t position, int frameNumber, int ent_num, vec3_t rayStart, vec3_t rayEnd, vec3_t scale, int traceFlags, int use_lod, float fRadius) {
	if (!ghoul2) return;
	re->G2API_CollisionDetectCache(collRecMap, *static_cast<CGhoul2Info_v*>(ghoul2), angles, position, frameNumber, ent_num, rayStart, rayEnd, scale, G2VertSpaceClient, traceFlags, use_lod, fRadius);
}
...
static qboolean CL_G2API_SetBoneAngles(void* ghoul2, int model_index, const char* bone_name, const vec3_t angles, const int flags, const int up, const int right, const int forward, qhandle_t* model_list, int blend_time, int current_time) {
	if (!ghoul2) return qfalse;
	return re->G2API_SetBoneAngles(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bone_name, angles, flags, static_cast<const Eorientations>(up), static_cast<const Eorientations>(right), static_cast<const Eorientations>(forward), model_list, blend_time, current_time);
}

static qboolean CL_G2API_SetBoneAnim(void* ghoul2, const int model_index, const char* bone_name, const int start_frame, const int end_frame, const int flags, const float anim_speed, const int current_time, const float setFrame, const int blend_time) {
	if (!ghoul2) return qfalse;
	return re->G2API_SetBoneAnim(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bone_name, start_frame, end_frame, flags, anim_speed, current_time, setFrame, blend_time);
}

static qboolean CL_G2API_GetBoneAnim(void* ghoul2, const char* bone_name, const int current_time, float* current_frame, int* start_frame, int* end_frame, int* flags, float* anim_speed, int* model_list, const int model_index) {
	if (!ghoul2) return qfalse;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_GetBoneAnim(g2, model_index, bone_name, current_time, current_frame, start_frame, end_frame, flags, anim_speed, model_list);
}

static qboolean CL_G2API_GetBoneFrame(void* ghoul2, const char* bone_name, const int current_time, float* current_frame, int* model_list, const int model_index) {
	if (!ghoul2) return qfalse;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	int iDontCare1 = 0, iDontCare2 = 0, iDontCare3 = 0;
	float fDontCare1 = 0;

	return re->G2API_GetBoneAnim(g2, model_index, bone_name, current_time, current_frame, &iDontCare1, &iDontCare2, &iDontCare3, &fDontCare1, model_list);
}
...
static qboolean CL_G2API_HasGhoul2ModelOnIndex(void* ghl_info, int model_index) {
	return re->G2API_HasGhoul2ModelOnIndex(static_cast<CGhoul2Info_v**>(ghl_info), model_index);
}

static qboolean CL_G2API_RemoveGhoul2Model(void* ghl_info, int model_index) {
#ifdef _FULL_G2_LEAK_CHECKING
	g_G2AllocServer = 0;
#endif
...
static qboolean CL_G2API_SkinlessModel(void* ghl_info, int model_index) {
...
static int CL_G2API_GetNumGoreMarks(void* ghl_info, int model_index) {
#ifdef _G2_GORE
...
#ifdef _G2_GORE
	if (!ghl_info) return;
	re->G2API_AddSkinGore(*static_cast<CGhoul2Info_v*>(ghl_info), *(SSkinGoreData*)gore);
#endif
}
...
static int CL_G2API_AddBolt(void* ghoul2, int model_index, const char* bone_name) {
	if (!ghoul2) return 0;
	return re->G2API_AddBolt(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bone_name);
}

static qboolean CL_G2API_AttachEnt(int* bolt_info, void* ghlInfoTo, int toBoltIndex, int ent_num, int toModelNum) {
...
static void CL_G2API_SetBoltInfo(void* ghoul2, int model_index, int bolt_info) {
	if (!ghoul2) return;
...
static qboolean CL_G2API_DoesBoneExist(void* ghoul2, int model_index, const char* bone_name) {
	if (!ghoul2) return qfalse;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static qboolean CL_G2API_GetRagBonePos(void* ghoul2, const char* bone_name, vec3_t pos, vec3_t entAngles, vec3_t entPos, vec3_t entScale) {
	if (!ghoul2) return qfalse;
...
static qboolean CL_G2API_SetBoneIKState(void* ghoul2, int time, const char* bone_name, int ikState, sharedSetBoneIKStateParams_t* params) {
	if (!ghoul2) return qfalse;
...
static qboolean CL_G2API_RemoveBone(void* ghoul2, const char* bone_name, int model_index) {
	if (!ghoul2) return qfalse;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_RemoveBone(g2, model_index, bone_name);
}

static void CL_G2API_AttachInstanceToEntNum(void* ghoul2, int entity_num, qboolean server) {
	if (!ghoul2) return;
	re->G2API_AttachInstanceToEntNum(*static_cast<CGhoul2Info_v*>(ghoul2), entity_num, server);
}

static void CL_G2API_ClearAttachedInstance(int entity_num) {
...
static void CL_G2API_GetSurfaceName(void* ghoul2, int surf_number, int model_index, char* fillBuf) {
	if (!ghoul2) return;
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	const char* tmp = re->G2API_GetSurfaceName(g2, model_index, surf_number);
	strcpy(fillBuf, tmp);
}
