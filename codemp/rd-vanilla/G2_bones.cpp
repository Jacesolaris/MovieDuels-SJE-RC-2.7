	...
	const Eorientations up, const Eorientations left, const Eorientations forward)
{
	...
	if (flags & (BONE_ANGLES_PREMULT | BONE_ANGLES_POSTMULT))
	{
		...
		const mdxaSkelOffsets_t* offsets = (mdxaSkelOffsets_t*)((byte*)mod->mdxa + sizeof(mdxaHeader_t));
		mdxaSkel_t* skel = (mdxaSkel_t*)((byte*)mod->mdxa + sizeof(mdxaHeader_t) + offsets->offsets[blist[index].
			boneNumber]);
...
qboolean G2_Set_Bone_Angles_Index(boneInfo_v& blist, const int index,
	...
	const Eorientations pitch, const Eorientations roll, qhandle_t* model_list,
...
qboolean G2_Set_Bone_Angles(CGhoul2Info* ghl_info, boneInfo_v& blist, const char* bone_name, const float* angles,
	...
	qhandle_t* model_list, const int model_index, const int blend_time, const int current_time)
{
...
qboolean G2_Set_Bone_Angles_Matrix_Index(boneInfo_v& blist, const int index,
	const mdxaBone_t& matrix, const int flags, qhandle_t* model_list,
	const int model_index, const int blend_time, const int current_time)
{
	if ((index >= static_cast<int>(blist.size())) || (blist[index].boneNumber == -1))
	{
...
// Given a model handle, and a bone name, we want to set angles specifically for overriding - using a matrix directly
qboolean G2_Set_Bone_Angles_Matrix(const char* fileName, boneInfo_v& blist, const char* bone_name, const mdxaBone_t& matrix,
	const int flags, qhandle_t* model_list, const int model_index, const int blend_time, const int current_time)
{
	model_t* mod_m;
	if (!fileName[0])
	{
	...
	const int num_frames)
{
	...
	if (flags & BONE_ANIM_BLEND)
	{
		...
		if (G2_Get_Bone_Anim_Index(blist, index, current_time, &current_frame, &start_frame, &end_frame, &flags, &anim_speed, nullptr, num_frames))
		{
...
// given a model, bone name, a bonelist, a start/end frame number, a anim speed and some anim flags, set up or modify an existing bone entry for a new set of anims
qboolean G2_Set_Bone_Anim(CGhoul2Info* ghl_info,
	boneInfo_v& blist,
...
// given a model, bonelist and bonename, return the current frame, startframe and endframe of the current animation
// NOTE if we aren't running an animation, then qfalse is returned
qboolean G2_Get_Bone_Anim(CGhoul2Info* ghl_info, boneInfo_v& blist, const char* bone_name, const int current_time,
	float* current_frame, int* start_frame, int* end_frame, int* flags, float* retAnimSpeed, qhandle_t* model_list, int model_index)
{
	const model_t* mod_a = (model_t*)ghl_info->animModel;
	...
	if (G2_Get_Bone_Anim_Index(blist, index, current_time, current_frame, start_frame, end_frame, flags, retAnimSpeed, model_list, ghl_info->aHeader->num_frames))
	{
...
qboolean G2_Pause_Bone_Anim(CGhoul2Info* ghl_info, boneInfo_v& blist, const char* bone_name, const int current_time)
{
	const model_t* mod_a = (model_t*)ghl_info->animModel;
	...
	if (index != -1)
	{
		...
		if (blist[index].pauseTime)
		{
			...
			G2_Get_Bone_Anim(ghl_info, blist, bone_name, blist[index].pauseTime, &current_frame, &start_frame, &end_frame, &flags, &anim_speed, nullptr, 0);
				...
				if (animSize)
				{
						...
						((anim_speed < 0.0f) && (newFrame_g < end_frame + 1)))
					{
						...
						if (blist[i].flags & BONE_ANIM_OVERRIDE_LOOP)
						{
							...
							blist[i].startTime = current_time - static_cast<int>((frameTime / anim_speed) * 50.0f);
...
*/
static void G2_RagDollSolve(CGhoul2Info_v& ghoul2V, int g2Index, float decay, int frame_num, const vec3_t currentOrg, bool limit_angles, CRagDollUpdateParams* params = nullptr);
static void G2_RagDollCurrentPosition(CGhoul2Info_v& ghoul2V, int g2Index, int frame_num, const vec3_t angles, const vec3_t position, const vec3_t scale);
...
int G2_Find_Bone_Rag(CGhoul2Info* ghl_info, const boneInfo_v& blist, const char* bone_name)
{
	...
	*/
	...
	for (size_t i = 0; i < blist.size(); i++)
	{
		...
		// figure out what skeletal info structure this bone entry is looking at
		const mdxaSkel_t* skel = (mdxaSkel_t*)((byte*)ghl_info->aHeader + sizeof(mdxaHeader_t) + offsets->offsets[blist[i].
			boneNumber]);
...
qboolean G2_Set_Bone_Anim_No_BS(CGhoul2Info& ghoul2, const mdxaHeader_t* mod, boneInfo_v& blist, const char* bone_name, const int argStartFrame,
	const int argEndFrame, const int flags, const float anim_speed, const int current_time, const float setFrame,
...
//	G2_Set_Bone_Angles_Rag(ghoul2, mod_a,blist,"ltibia",RAG_EFFECTOR|RAG_BONE_LIGHTWEIGHT,(4.0f*sRadELeg)*fRadScale);
	...
	{
		G2_RagDollSettlePositionNumeroTrois(ghoul2V, dPos, &fparms, cur_time);

		G2_RagDollCurrentPosition(ghoul2V, model, cur_time, parms->angles, dPos, parms->scale);
		G2_RagDollMatchPosition();
		G2_RagDollSolve(ghoul2V, model, 1.0f * (1.0f - k / 40.0f), cur_time, dPos, false);
	}
}
...
void G2_SetRagDollBullet(CGhoul2Info& ghoul2, const vec3_t rayStart, const vec3_t hit)
{
	...
	{
		const int		magicFactor13 = 150.0f; // squared radius multiplier for shot effects
		boneInfo_v& blist = ghoul2.mBlist;
...
static float G2_RagSetState(CGhoul2Info& ghoul2, boneInfo_t& bone, int frame_num, const vec3_t origin, bool& resetOrigin)
{
	...
	{
		decay = 1.0f - (frame_num - bone.restTime) / static_cast<float>(dynamicTime);
		if (decay < 0.0f)
		{
			decay = 0.0f;
		}
		...
		const float	magicFactor8 = 1.0f; // Power for decay
...
static void G2_RagDoll(CGhoul2Info_v& ghoul2V, int g2Index, CRagDollUpdateParams* params, int cur_time)
{
	if (!broadsword || !broadsword->integer)
	{
		return;
	...
	if (iters)
	{
		...
		{
			G2_RagDollCurrentPosition(ghoul2V, g2Index, frame_num, params->angles, dPos, params->scale);

			if (G2_RagDollSettlePositionNumeroTrois(ghoul2V, dPos, params, cur_time))
			{
#if 0
			...
			G2_RagDollSolve(ghoul2V, g2Index, decay * 2.0f, frame_num, dPos, true, params);
		}
	}
...
#endif
		G2_RagDollCurrentPosition(ghoul2V, g2Index, frame_num, params->angles, params->position, params->scale);
...
static inline char* G2_Get_Bone_Name(CGhoul2Info* ghl_info, boneInfo_v& blist, int bone_num)
{
	...
	for (size_t i = 0; i < blist.size(); i++)
	{
		...
		// figure out what skeletal info structure this bone entry is looking at
		mdxaSkel_t* skel = (mdxaSkel_t*)((byte*)ghl_info->aHeader + sizeof(mdxaHeader_t) + offsets->offsets[blist[i].
			boneNumber]);
...
static void G2_RagDollCurrentPosition(CGhoul2Info_v& ghoul2V, int g2Index, int frame_num, const vec3_t angles, const vec3_t position, const vec3_t scale)
{
	...
	G2_ConstructGhoulSkeleton(ghoul2V, frame_num, false, scale);
...
static void G2_RagDollSolve(CGhoul2Info_v& ghoul2V, int g2Index, float decay, int frame_num, const vec3_t currentOrg, bool limit_angles, CRagDollUpdateParams* params)
{
	CGhoul2Info& ghoul2 = ghoul2V[g2Index];
...
static void G2_IKSolve(CGhoul2Info_v& ghoul2V, int g2Index, float decay, int frame_num, const vec3_t currentOrg, bool limitAngles)
{
	CGhoul2Info& ghoul2 = ghoul2V[g2Index];
...
static void G2_DoIK(CGhoul2Info_v& ghoul2V, int g2Index, CRagDollUpdateParams* params)
{
	...
	if (iters)
	{
		...
		for (int i = 0; i < iters; i++)
		{
			...
			G2_IKSolve(ghoul2V, g2Index, decay * 2.0f, frame_num, params->position, true);
		}
	}
