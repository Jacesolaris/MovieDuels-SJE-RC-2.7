...
static void R_MipMap2(unsigned* in, int inWidth, int inHeight) {
	...
	unsigned* temp = static_cast<unsigned*>(Hunk_AllocateTempMemory(outWidth * outHeight * 4));
...
static void R_CreateFogImage(void) {
	...
	byte* data = static_cast<unsigned char*>(Hunk_AllocateTempMemory(FOG_S * FOG_T * 4));
...
void R_SetGammaCorrectionLUT()
{
	if (glConfigExt.doGammaCorrectionWithShaders)
	{
		...
		for (int i = 0; i < 64; i++)
		{
			if (g == 1.0f)
			{
				inf = static_cast<int>(((float)i / 63.0f) * 255.0f + 0.5f);
			}
			else
			{
				inf = static_cast<int>(255.0f * pow((float)i / 63.0f, 1.0f / g) + 0.5f);
			}

			gammaCorrected[i] = Com_Clampi(0, 255, inf << shift);
		}

		byte* lutTable = static_cast<byte*>(ri->Hunk_AllocateTempMemory(64 * 64 * 64 * 3));
		byte* write = lutTable;
		for (int z = 0; z < 64; z++)
		{
			for (int y = 0; y < 64; y++)
