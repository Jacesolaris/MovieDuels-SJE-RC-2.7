...
static void R_SetupEntityLightingGrid(trRefEntity_t* ent) {
	...
	unsigned short* startGridPos = tr.world->lightGridArray + (pos[0] * gridStep[0] + pos[1] * gridStep[1] + pos[2] *
		gridStep[2]);
	...
	for (i = 0; i < 8; i++) {
		...
		float factor = 1.0;
		unsigned short* gridPos = startGridPos;
		for (j = 0; j < 3; j++) {
			if (i & (1 << j)) {
				factor *= frac[j];
				gridPos += gridStep[j];
		...
		const mgrid_t* data = tr.world->lightGridData + *gridPos;
		...
		int lat = data->latLong[1];
		int lng = data->latLong[0];
