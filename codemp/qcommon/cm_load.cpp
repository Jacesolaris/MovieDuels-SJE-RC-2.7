...
static void CMod_LoadSubmodels(const lump_t* l, clipMap_t& cm) {
	...
	cm.cmodels = (cmodel_s*)Hunk_Alloc(count * sizeof(*cm.cmodels), h_high);
	...
	for (int i = 0; i < count; i++, in++, out++)
	{
		...
		int* indexes = (int*)Hunk_Alloc(out->leaf.numLeafBrushes * 4, h_high);
...
static void CMod_LoadPlanes(const lump_t* l, clipMap_t& cm)
{
	...
	cm.planes = (cplane_s*)Hunk_Alloc((BOX_PLANES + count) * sizeof(*cm.planes), h_high);
	cm.numPlanes = count;
...
{ //rwwRMG - function needs heavy modification
	...
	if (!name[0]) {
		...
		cm.numClusters = 1;
		cm.numAreas = 1;
		cm.cmodels = (cmodel_s*)Hunk_Alloc(sizeof(*cm.cmodels), h_high);
		if (checksum)
			*checksum = 0;
		return;
	}
