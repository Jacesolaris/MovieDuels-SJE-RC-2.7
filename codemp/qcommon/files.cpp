...
qboolean FS_Initialized(void) {
	return (qboolean)(fs_searchpaths != nullptr);
}
...
static pack_t* FS_LoadZipFile(const char* zipfile, const char* basename)
{
	...
	fileInPack_t* buildBuffer = (struct fileInPack_s*)Z_Malloc((gi.number_entry * sizeof(fileInPack_t)) + len,
		TAG_FILESYS, qtrue);
	char* namePtr = ((char*)buildBuffer) + gi.number_entry * sizeof(fileInPack_t);
	int* fs_headerLongs = (int*)Z_Malloc((gi.number_entry + 1) * sizeof(int), TAG_FILESYS, qtrue);
	...
	pack_t* pack = (pack_t*)Z_Malloc(sizeof(pack_t) + i * sizeof(fileInPack_t*), TAG_FILESYS, qtrue);
	pack->hashSize = i;
...
static char** Sys_ConcatenateFileLists(char** list0, char** list1, char** list2)
{
	...
	/* Create new list. */
	char** dst = cat = (char**)Z_Malloc((totalLength + 1) * sizeof(char*), TAG_FILESYS, qtrue);
...
void FS_SortFileList(char** filelist, int numfiles) {
	...
	char** sortedlist = (char**)Z_Malloc((numfiles + 1) * sizeof(*sortedlist), TAG_FILESYS, qtrue);
...
static void FS_AddGameDirectory(const char* path, const char* dir) {
	...
	//
	searchpath_t* search = (struct searchpath_s*)Z_Malloc(sizeof(searchpath_t), TAG_FILESYS, qtrue);
