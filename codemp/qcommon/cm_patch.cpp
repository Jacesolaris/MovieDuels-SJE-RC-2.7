...
==================
*/
static inline int CM_PointOnPlaneSide(const float* p, int planeNum) {
	if (planeNum == -1) {
		return SIDE_ON;
	}
...
patchCollide_s* CM_GeneratePatchCollide(int width, int height, vec3_t* points) {
	...
	// collided against
	patchCollide_t* pf = (struct patchCollide_s*)Hunk_Alloc(sizeof(*pf), h_high);
...
====================
*/
static inline int CM_CheckFacetPlane(const float* plane, vec3_t start, vec3_t end, float* enterFrac, float* leaveFrac, int* hit) {
	float f;
