...
void G2_RagGetBoneBasePoseMatrixLow(const CGhoul2Info& ghoul2, int boneNum, const mdxaBone_t& boneMatrix, mdxaBone_t& retMatrix, vec3_t scale)
{
	assert(ghoul2.mBoneCache);
	const CBoneCache& boneCache = *ghoul2.mBoneCache;
	assert(boneCache.mod);
...
void G2_GetBoneMatrixLow(const CGhoul2Info& ghoul2, int boneNum, const vec3_t scale, mdxaBone_t& retMatrix, mdxaBone_t*& retBasepose, mdxaBone_t*& retBaseposeInv)
{
	if (!ghoul2.mBoneCache)
	{
...
int G2_GetParentBoneMatrixLow(const CGhoul2Info& ghoul2, int boneNum, const vec3_t scale, mdxaBone_t& retMatrix, mdxaBone_t*& retBasepose, mdxaBone_t*& retBaseposeInv)
{
	int parent = -1;
	if (ghoul2.mBoneCache)
	{
...
void G2_TransformBone(int child, CBoneCache& BC)
{
	...
	offsets = reinterpret_cast<mdxaSkelOffsets_t*>((byte*)BC.header + sizeof(mdxaHeader_t));
	skel = reinterpret_cast<mdxaSkel_t*>((byte*)BC.header + sizeof(mdxaHeader_t) + offsets->offsets[child]);
...
void G2_TransformGhoulBones(boneInfo_v& rootBoneList, const mdxaBone_t& rootMatrix, CGhoul2Info& ghoul2, int time, bool smooth = true)
{
#ifdef G2_PERFORMANCE_ANALYSIS
	G2PerformanceCounter_G2_TransformGhoulBones++;
#endif
...
void G2_ProcessSurfaceBolt2(CBoneCache& boneCache, const mdxmSurface_t* surface, int boltNum, boltInfo_v& boltList, const surfaceInfo_t* surfInfo, const model_t* mod, mdxaBone_t& retMatrix)
{
	...
	if (surfInfo && surfInfo->offFlags == G2SURFACEFLAG_GENERATED)
	{
		...
		mdxmSurface_t* originalSurf = static_cast<mdxmSurface_t*>(G2_FindSurface(mod, surfNumber, surfInfo->genLod));
...
void RenderSurfaces(CRenderSurface& RS)
{
	...
	mdxmHierarchyOffsets_t* surfIndexes = reinterpret_cast<mdxmHierarchyOffsets_t*>(reinterpret_cast<byte*>(RS.currentModel->mdxm) + sizeof(mdxmHeader_t));
