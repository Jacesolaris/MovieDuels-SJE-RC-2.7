...
static void RB_VerticalSurfaceSprite(vec3_t loc, float width, float height, byte light,
	byte alpha, float wind, const float windidle, vec2_t fog, const int hangdown, vec2_t skew, bool flattened)
{
	vec3_t loc2, right;
	float windsway;
	float points[16];
...
static void RB_VerticalSurfaceSpriteWindPoint(vec3_t loc, float width, float height, byte light,
	byte alpha, float wind, const float windidle, vec2_t fog,
	const int hangdown, vec2_t skew, vec2_t winddiff, float windforce, bool flattened)
{
	vec3_t loc2, right;
	float points[16];
	color4ub_t color;
...
static void RB_OrientedSurfaceSprite(vec3_t loc, float width, float height, byte light, byte alpha, vec2_t fog, const int faceup)
{
	float points[16];
	color4ub_t color;
...
static void RB_EffectSurfaceSprite(vec3_t loc, float width, float height, byte light, byte alpha, float life, const int faceup)
{
	float points[16];
	color4ub_t color;
...
extern void R_InvertMatrix(const float* sourcemat, float* destmat);
