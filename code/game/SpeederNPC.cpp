...
bool Update(Vehicle_t* pVeh, const usercmd_t* pUcmd)
{
	...
	if (pVeh->m_pVehicleInfo->iExhaustFX)
	{
		...
		if (pVeh->m_ucmd.forwardmove && !(pVeh->m_ulFlags & VEH_ACCELERATORON))
		{
			...
			if (speed < 100.0f)
			{
				if (pVeh->m_iSoundDebounceTimer < level.time)
				{
					if (pVeh->m_pVehicleInfo->soundAccelerate)
					{
						pVeh->m_iSoundDebounceTimer = level.time + Q_irand(1000, 3000);
						G_SoundIndexOnEnt(pVeh->m_pParentEntity, CHAN_AUTO, pVeh->m_pVehicleInfo->soundAccelerate);
					}
				}
			}
		}
		...
		else if (!pVeh->m_ucmd.forwardmove && pVeh->m_ulFlags & VEH_ACCELERATORON)
		{
			...
			for (int i = 0; i < MAX_VEHICLE_EXHAUSTS && pVeh->m_iExhaustTag[i] != -1; i++)
			{
				...
				if (speed > 100.0f)
				{
					if (pVeh->m_pVehicleInfo->soundBraking)
					{
						G_SoundIndexOnEnt(pVeh->m_pParentEntity, CHAN_AUTO, pVeh->m_pVehicleInfo->soundBraking);
					}
				}
			}
		}
		...
		{
			if (pVeh->m_ucmd.forwardmove) // holding forward accelerate and going at speed
			{
				if (speed > 300.0f)
				{
					if (pVeh->m_iSoundDebounceTimer < level.time && Q_irand(0, 1) == 0)
	...
	)
	{
		...
		if (curTime - pVeh->m_iTurboTime > pVeh->m_pVehicleInfo->turboRecharge)
		{
			...
			if (pVeh->m_pVehicleInfo->soundTurbo)
			{
				...
				{
				case 1:
					turbo_sound = pVeh->m_pVehicleInfo->soundTurbo;
					break;
				case 2:
					turbo_sound = pVeh->m_pVehicleInfo->soundTurbo2;
					break;
				case 3:
					turbo_sound = pVeh->m_pVehicleInfo->soundTurbo3;
					break;
				case 4:
					turbo_sound = pVeh->m_pVehicleInfo->soundTurbo4;
					break;
				default: ;
				}
				if (turbo_sound)
				{
					G_SoundIndexOnEnt(pVeh->m_pParentEntity, CHAN_AUTO, turbo_sound);
				}
			}
#endif
	...
	if (pVeh->m_iBoarding != 0)
	{
		...
		if (pVeh->m_iBoarding < 0)
		{
...
#else
			if (pVeh->m_iBoarding == BOTH_VS_MOUNTJUMP_L)
			{
				i_anim_len = PM_AnimLength(pVeh->m_pPilot->client->clientInfo.animFileIndex, Anim);
			}
			else
			{
				i_anim_len = PM_AnimLength(pVeh->m_pPilot->client->clientInfo.animFileIndex, Anim); // * 0.4f;
			}
			...
			if (pVeh->m_iBoarding != VEH_MOUNT_THROW_LEFT && pVeh->m_iBoarding != VEH_MOUNT_THROW_RIGHT)
			{
				if (pVeh->m_iBoarding == BOTH_VS_MOUNTJUMP_L)
				{
					pVeh->m_iBoarding = level.time + i_anim_len;
				}
				else
				{
					pVeh->m_iBoarding = level.time + i_anim_len * 0.4f;
				}
			}
			else
			{
			...
			if (pVeh->m_pOldPilot)
			{
				PM_AnimLength(pVeh->m_pPilot->client->clientInfo.animFileIndex, BOTH_VS_MOUNTTHROWEE);
