...
static void IT_Name(const char** holdBuf)
{
	...
	else if (!Q_stricmp(tokenStr, "ITM_DUALPISTOL_PICKUP"))
	...
	else if (!Q_stricmp(tokenStr, "ITM_GRAPPLEHOOK_PICKUP"))
...
static void IT_Tag(const char** holdBuf)
{
	...
	if (!Q_stricmp(tokenStr, "WP_NONE"))
		...
		tag = WP_SBD_BLASTER;
	else if (!Q_stricmp(tokenStr, "WP_DUAL_PISTOL"))
		tag = WP_DUAL_PISTOL;
	...
	else if (!Q_stricmp(tokenStr, "FP_PROJECTION"))
	{
		tag = FP_PROJECTION;
	}
	else if (!Q_stricmp(tokenStr, "ITM_BATTERY_PICKUP"))
	...
	else if (!Q_stricmp(tokenStr, "INV_GRAPPLEHOOK"))
	{
		tag = INV_GRAPPLEHOOK;
	}
	else
