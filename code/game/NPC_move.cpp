...
qboolean npc_check_fall_position_ok(const gentity_t* npc, vec3_t position)
{
	trace_t tr;
	vec3_t test_pos, down_pos;
	vec3_t mins, maxs;
	...
	gi.trace(&tr, test_pos, mins, maxs, down_pos, npc->s.number, MASK_PLAYERSOLID, static_cast<EG2_Collision>(0), 0);
