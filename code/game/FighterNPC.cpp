...
qboolean FighterIsLanded(const Vehicle_t* pVeh, playerState_t* parentPS)
{
...
static void ProcessMoveCommands(Vehicle_t* pVeh)
{
	...
	is_landing_or_launching = static_cast<qboolean>(FighterIsLanding(pVeh, parentPS) ||
		...
		pVeh->m_ucmd.forwardmove || pVeh->m_ucmd.upmove > 0) && pVeh->m_LandTrace.fraction >= 0.05f)
	{
		...
		else if (pVeh->m_ucmd.forwardmove < 0 || pVeh->m_ucmd.upmove < 0)
		{
			...
			if (pVeh->m_pVehicleInfo->type == VH_FIGHTER && pVeh->m_pVehicleInfo->Inhabited(pVeh))
			{
				pVeh->m_ucmd.forwardmove = 127;
			}
			else if (speedMin >= 0)
                                 ...
                                 const qboolean isDead)
{
	...
	if (pVeh->m_LandTrace.fraction < 1.0f)
	{
		...
		auto targ = pVeh->m_pParentEntity;
		gentity_t* killer = targ;
#ifdef _JK2MP//only have this info in MP...
		if (targ->client->ps.otherKiller < ENTITYNUM_WORLD &&
			targ->client->ps.otherKillerTime > level.time)
		{
			gentity_t* potentialKiller = &g_entities[targ->client->ps.otherKiller];
...
#endif
		G_Damage(targ, killer, killer, vec3_origin, targ->client->ps.origin, 99999, DAMAGE_NO_ARMOR, MOD_SUICIDE);
	}
#endif
...
static void ProcessOrientCommands(Vehicle_t* pVeh)
{
	...
	is_dead = static_cast<qboolean>(parentPS->stats[STAT_HEALTH] <= 0);
#endif
	...
	if (is_dead || parentPS->electrifyTime >= curTime ||
		...
		pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_F)
	{
		...
		FighterDamageRoutine(pVeh, parent, parentPS, riderPS, is_dead);
	...
	is_landing_or_landed = static_cast<qboolean>(FighterIsLanding(pVeh, parentPS) || FighterIsLanded(pVeh, parentPS));
	...
	//&& !( pVeh->m_ucmd.forwardmove > 0 && pVeh->m_LandTrace.fraction != 1.0f ) )
	{
		...
		else
		{
			...
			//if ( ( pVeh->m_LandTrace.fraction >= 1.0f || pVeh->m_ucmd.forwardmove != 0 ) && pVeh->m_LandTrace.fraction >= 0.0f )
			{
				...
				cur_roll = PredictedAngularDecrement(0.93f, angleTimeMod * 2.0f, cur_roll);
				//cap it reasonably
				//NOTE: was hardcoded to 40.0f, now using extern data
				if (pVeh->m_pVehicleInfo->rollLimit != -1)
				{
					if (cur_roll > pVeh->m_pVehicleInfo->rollLimit)
					{
						cur_roll = pVeh->m_pVehicleInfo->rollLimit;
					}
					else if (cur_roll < -pVeh->m_pVehicleInfo->rollLimit)
					{
						cur_roll = -pVeh->m_pVehicleInfo->rollLimit;
					}
				}
	...
	{
		//use that roll
		pVeh->m_vOrientation[ROLL] = cur_roll;
...
{
	int Anim = -1;
	constexpr int iFlags = SETANIM_FLAG_NORMAL;
	playerState_t* parentPS = &pVeh->m_pParentEntity->client->ps;
