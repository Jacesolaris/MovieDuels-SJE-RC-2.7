...
//
////////////////////////////////////////////////////////////////////////////////////////
class CGraphUser final : public TGraph::user
{
...
bool NAV::TestEdge(const TNodeHandle NodeA, const TNodeHandle NodeB, const qboolean IsDebugEdge)
{
		...
		&g_entities[EntHit] != nullptr)
	{
		...
		if (CanGo && !HitCharacter)
		{
			...
			if (at.BlockingDoor())
			{
				// Doors Need To Know Their "owner" Entity - The Thing That controls
...
//
////////////////////////////////////////////////////////////////////////////////////////
void NAV::RegisterDangerSense(const gentity_t* actor, const int alertEventIndex)
{
...
void NAV::SpawnedPoint(gentity_t* ent, const EPointType type)
{
	...
	const int NodeHandle = mGraph.insert_node(node);
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::ChooseRandomNeighbor(const TNodeHandle NodeHandle, const vec3_t& position, const float maxDistance)
{
	if (NodeHandle != WAYPOINT_NONE && NodeHandle > 0)
	{
...
NAV::TNodeHandle NAV::ChooseClosestNeighbor(const TNodeHandle NodeHandle, const vec3_t& position)
{
	if (NodeHandle != WAYPOINT_NONE && NodeHandle > 0)
	{
		...
		for (int i = 0; i < neighbors.size(); i++)
		{
			...
			const float CurDist = mGraph.get_node(Cur).mPoint.Dist2(pos);
...
//
////////////////////////////////////////////////////////////////////////////////////////
const vec3_t& NAV::NextPosition(const gentity_t* actor)
{
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::NextPosition(const gentity_t* actor, CVec3& Position)
{
	assert(HasPath(actor));
	TPath& path = mPathUsers[mPathUserIndex[actor->s.number]].mPath;
...
////////////////////////////////////////////////////////////////////////////////////////
void NAV::ClearPath(const gentity_t* actor)
{
...
bool NAV::UpdatePath(gentity_t* actor, const TNodeHandle target, const float MaxDangerLevel)
{
	...
	do
	{
		...
		in_reached_radius = Dir.Len2() < PPoint.mReachedRadius;
		if (in_reached_radius)
		{
			ReachedAnything = true;
			path.pop_back();
		}
	} while (in_reached_radius && path.size() > 0);
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::HasPath(const gentity_t* actor, const TNodeHandle target)
{
	const int pathUserNum = mPathUserIndex[actor->s.number];
	if (pathUserNum == NULL_PATH_USER_INDEX)
	{
...
float NAV::PathDangerLevel(gentity_t* actor)
{
	...
	for (; curPathAt >= 0; curPathAt--)
	{
		...
		if (prevNode < 0 || mGraph.get_edge_across(prevNode, PPoint.mNode))
		{
			...
			for (int alIndex = 0; alIndex < TAlertList::CAPACITY; alIndex++)
			{
				if (al[alIndex].mHandle == cur_edge && al[alIndex].mDanger > DangerLevel)
				{
		...
		{
			const float enemyDangerLevel = enemyPos.DistToLine(prevPoint, PPoint.mPoint) / enemySafeDist;
...
//
////////////////////////////////////////////////////////////////////////////////////////
int NAV::PathNodesRemaining(const gentity_t* actor)
{
	const int pathUserNum = mPathUserIndex[actor->s.number];
	if (pathUserNum == NULL_PATH_USER_INDEX)
	{
...
////////////////////////////////////////////////////////////////////////////////////////
unsigned int NAV::ClassifyEntSize(const gentity_t* ent)
{
	if (ent)
	{
...
void NAV::ShowDebugInfo(const vec3_t& PlayerPosition, int player_waypoint)
{
	mUser.ClearActor();
...
// Activate
////////////////////////////////////////////////////////////////////////////////////
void STEER::Activate(const gentity_t* actor)
{
	...
	const CVec3 Range(RangeSize, RangeSize,
...
//
//////////////////////////////////////////////////////////////////////
bool STEER::Active(const gentity_t* actor)
{
...
// Blocked Recorder
////////////////////////////////////////////////////////////////////////////////////
void STEER::Blocked(const gentity_t* actor, const gentity_t* target)
{
	assert(Active(actor));
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
// Blocked Recorder
////////////////////////////////////////////////////////////////////////////////////
void STEER::Blocked(const gentity_t* actor, const vec3_t& target)
{
	assert(Active(actor));
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
// Blocked Recorder
////////////////////////////////////////////////////////////////////////////////////
bool STEER::HasBeenBlockedFor(const gentity_t* actor, const int duration)
{
...
//
//////////////////////////////////////////////////////////////////////
float STEER::MatchSpeed(const gentity_t* actor, const float speed, const float weight)
{
	assert(Active(actor));
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Seek(const gentity_t* actor, const CVec3& pos, const float slowingDistance, const float weight, const float desiredSpeed)
{
	assert(Active(actor));
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Flee(const gentity_t* actor, const CVec3& pos, const float weight)
{
	assert(Active(actor));
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Persue(const gentity_t* actor, gentity_t* target, const float slowingDistance)
{
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Evade(const gentity_t* actor, gentity_t* target)
{
	assert(Active(actor) && target);
	const SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
// Separation
////////////////////////////////////////////////////////////////////////////////////
float STEER::Separation(const gentity_t* actor, const float Scale)
{
...
// Cohesion
////////////////////////////////////////////////////////////////////////////////////
float STEER::Cohesion(const gentity_t* actor, float Scale)
{
	assert(Active(actor) && actor);
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
// Find Nearest Leader
////////////////////////////////////////////////////////////////////////////////////
gentity_t* STEER::SelectLeader(const gentity_t* actor)
{
	assert(Active(actor) && actor);
	SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
// Wander
////////////////////////////////////////////////////////////////////////////////////
float STEER::Wander(const gentity_t* actor)
{
	assert(Active(actor) && actor);
	const SSteerUser& suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
...
////////////////////////////////////////////////////////////////////////////////////
float STEER::FollowLeader(const gentity_t* actor, gentity_t* leader, const float dist)
{
...
// away from them.
////////////////////////////////////////////////////////////////////////////////////
float STEER::AvoidCollisions(gentity_t* actor, const gentity_t* leader)
{
