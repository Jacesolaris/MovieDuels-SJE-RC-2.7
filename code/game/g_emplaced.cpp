	...
	if (!tr.startsolid && !tr.allsolid)
	{
		...
		if (!tr.startsolid && !tr.allsolid) //&& tr.fraction == 1.0f)
		{
			...
			if (move_dist > 4.0f)
			{
				...
				AngleVectors(owner->currentAngles, nullptr, o_right, nullptr);
				if (DotProduct(move_dir, o_right) > 0)
				{
					//moved to his right, play right strafe
					strafe_anim = BOTH_STRAFE_RIGHT1;
				}
				else
				{
					//moved left, play left strafe
					strafe_anim = BOTH_STRAFE_LEFT1;
				}
				NPC_SetAnim(owner, SETANIM_LEGS, strafe_anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			}
