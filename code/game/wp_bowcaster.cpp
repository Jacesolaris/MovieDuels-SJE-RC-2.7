	...
	for (int i = 0; i < count; i++)
	{
			...
			FORCE_LEVEL_2)
		{
			...
			else if (ent->client && ent->client->ps.BlasterAttackChainCount >= BLASTERMISHAPLEVEL_FULL)
			{
				...
				angs[PITCH] += Q_flrand(-3.0f, 3.0f) * BOWCASTER_ALT_SPREAD;
				angs[YAW] += Q_flrand(-3.0f, 3.0f) * BOWCASTER_ALT_SPREAD;
			}
			else if (ent->client && ent->client->ps.BlasterAttackChainCount >= BLASTERMISHAPLEVEL_HEAVY)
			{
				// add some slop to the fire direction
				angs[PITCH] += Q_flrand(-2.0f, 2.0f) * BOWCASTER_ALT_SPREAD;
				angs[YAW] += Q_flrand(-2.0f, 2.0f) * BOWCASTER_ALT_SPREAD;
			}
			...
			{
				angs[PITCH] += Q_flrand(-0.5f, 0.5f) * BOWCASTER_ALT_SPREAD;
				angs[YAW] += Q_flrand(-0.5f, 0.5f) * BOWCASTER_ALT_SPREAD;
			}
		}
