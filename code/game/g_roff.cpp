...
static void G_RoffNotetrackCallback(const gentity_t* cent, const char* notetrack)
{
	...
	defaultoffsetposition:
		...
		object_id = G_EffectIndex(argument);
		r = 0;
		...
		if (object_id)
		{
			...
			G_PlayEffect(object_id, useOrigin, useAngles);
		}
	}
	else if (strcmp(type, "sound") == 0)
	{
		object_id = G_SoundIndex(argument);
		cgi_S_StartSound(cent->s.pos.trBase, cent->s.number, CHAN_BODY, object_id);
	}
	//else if ...
	else
	{
...
static qboolean G_ValidRoff(roff_hdr2_t* header)
{
	if (!strncmp(header->mHeader, "ROFF", 4))
	{
		...
		if (LittleLong header->mVersion == ROFF_VERSION && LittleFloat reinterpret_cast<roff_hdr_t*>(header)->mCount >
			0.0f)
		{
