...
qboolean NAV_HitNavGoal(vec3_t point, vec3_t mins, vec3_t maxs, vec3_t dest, const int radius, const qboolean flying)
{
	...
	if (radius)
	{
		//NOTE:  This needs to do a DistanceSquared on nav goals that had
		//			a radius manually set! We can't do the smaller nav goals against
...
qboolean nav_check_ahead(const gentity_t* self, vec3_t end, trace_t& trace, int clipmask)
{
	vec3_t mins;
...
qboolean NPC_ClearPathToGoal(vec3_t dir, gentity_t* goal)
{
	...
	if (nav_check_ahead(NPC, goal->currentOrigin, trace, NPC->clipmask & ~CONTENTS_BODY | CONTENTS_BOTCLIP))
	{
