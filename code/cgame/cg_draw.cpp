...
================
*/
static void CG_DrawForcePower_md(const centity_t* cent, const float hud_ratio)
{
	...
	if (cg_SerenityJediEngineHudMode.integer == 3)
	{
		...
		CG_DrawNumField(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[OHB_FORCEAMOUNT_MD].xPos) + 4.5 * hud_ratio,
			otherHUDBits[OHB_FORCEAMOUNT_MD].yPos + 8,
			3,
			cent->gent->client->ps.forcePower,
			otherHUDBits[OHB_FORCEAMOUNT_MD].width * hud_ratio,
	...
	else
	{
		// Print force numeric amount
		CG_DrawNumField(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[OHB_FORCEAMOUNT_MD].xPos) * hud_ratio,
...
================
*/
static void CG_DrawSaberStyle_md(const centity_t* cent, const float hud_ratio)
{
	int index;
	...
	{
		CG_DrawPic(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[index].xPos) + 4.5 * hud_ratio,
			otherHUDBits[index].yPos + 8,
...
static void CG_DrawblockingMode(const centity_t* cent, const float hud_ratio)
{
	int blockindex;
	...
	{
		CG_DrawPic(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[blockindex].xPos) + 4.5 * hud_ratio,
			otherHUDBits[blockindex].yPos + 8,
			otherHUDBits[blockindex].width * hud_ratio,
	...
	{
		CG_DrawPic(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[blockindex].xPos) * hud_ratio,
...
================
*/
static void CG_Drawgunfatigue(const centity_t* cent, const float hud_ratio)
{
	vec4_t calc_color;
	constexpr int blaster_attack_chain_count = BLASTERMISHAPLEVEL_MAX;
	qboolean flash = qfalse;
		...
		BLASTERMISHAPLEVEL_FULL)
	{
		...
		if (cg.mishapHUDNextFlashTime < cg.time)
		{
			...
			cgi_S_StartSound(nullptr, 0, CHAN_AUTO, cgs.media.overheatgun);
	...
	constexpr float inc = static_cast<float>(blaster_attack_chain_count) / MAX_HUD_TICS;
...
static void CG_Drawsaberfatigue(const centity_t* cent, const float hud_ratio)
{
	vec4_t calc_color;
	...
	for (int i = MAX_SJEHUD_TICS - 1; i >= 0; i--)
	{
		...
		{
			CG_DrawPic(
				SCREEN_WIDTH - (SCREEN_WIDTH - sabfatticS[i].xPos) + 4.5 * hud_ratio,
				sabfatticS[i].yPos + 8,
				sabfatticS[i].width * hud_ratio,
				sabfatticS[i].height,
		...
		{
			CG_DrawPic(
				SCREEN_WIDTH - (SCREEN_WIDTH - sabfatticS[i].xPos) * hud_ratio,
				sabfatticS[i].yPos,
				sabfatticS[i].width * hud_ratio,
				sabfatticS[i].height,
				sabfatticS[i].background);
...
static void CG_DrawblockPoints(const int x, const int y, const float hud_ratio)
{
	vec4_t calc_color;
	...
	if (cg.predicted_player_state.ManualBlockingFlags & 1 << MBF_MBLOCKING)
	{
		...
		{
			//blockingflag is on
			CG_DrawPic(x - 4.5 * hud_ratio, y + 7, 40 * hud_ratio, 40, cgs.media.HUDblockpointMB1);
		}
		else
		{
			//blockingflag is on
			CG_DrawPic(x * hud_ratio, y, 40 * hud_ratio, 40, cgs.media.HUDblockpointMB1);
		}
	}
	else
	{
		if (cg_SerenityJediEngineHudMode.integer == 3)
		{
			CG_DrawPic(x - 4.5 * hud_ratio, y + 7, 40 * hud_ratio, 40, cgs.media.HUDblockpoint1);
		}
		else
		{
			CG_DrawPic(x * hud_ratio, y, 40 * hud_ratio, 40, cgs.media.HUDblockpoint1);
		}
	}
	...
	if (cg.predicted_player_state.ManualBlockingFlags & 1 << MBF_MBLOCKING)
	{
		...
		{
			//blockingflag is on
			CG_DrawPic(x - 4.5 * hud_ratio, y + 7, 40 * hud_ratio, 40, cgs.media.HUDblockpointMB2);
		}
		else
		{
			//blockingflag is on
			CG_DrawPic(x * hud_ratio, y, 40 * hud_ratio, 40, cgs.media.HUDblockpointMB2);
		}
	}
	else
	{
		if (cg_SerenityJediEngineHudMode.integer == 3)
		{
			CG_DrawPic(x - 4.5 * hud_ratio, y + 7, 40 * hud_ratio, 40, cgs.media.HUDblockpoint2);
		}
		else
		{
			CG_DrawPic(x * hud_ratio, y, 40 * hud_ratio, 40, cgs.media.HUDblockpoint2);
		}
	}
	...
	if (cg_SerenityJediEngineHudMode.integer == 3)
	{
		...
		CG_DrawNumField(
			otherHUDBits[OHB_BLOCKAMOUNT].xPos - 4.5 * hud_ratio,
			otherHUDBits[OHB_BLOCKAMOUNT].yPos + 7,
			3,
	...
	else
	{
		...
		CG_DrawNumField(
			otherHUDBits[OHB_BLOCKAMOUNT].xPos * hud_ratio,
...
================
*/
static void CG_DrawAmmo_md(const centity_t* cent, const float hud_ratio)
{
	vec4_t calc_color;
	...
	else
	{
		if (curr_value > 0)
		{
			...
			{
				memcpy(calc_color, otherHUDBits[OHB_AMMOAMOUNT_MD].color, sizeof(vec4_t));
			}
		}
		else
		{
	...
	{
		CG_DrawNumField(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[OHB_AMMOAMOUNT_MD].xPos) + 4.5 * hud_ratio,
			otherHUDBits[OHB_AMMOAMOUNT_MD].yPos + 8,
			3,
			ps->ammo[weaponData[cent->currentState.weapon].ammoIndex],
			otherHUDBits[OHB_AMMOAMOUNT_MD].width * hud_ratio,
	...
	{
		CG_DrawNumField(
			SCREEN_WIDTH - (SCREEN_WIDTH - otherHUDBits[OHB_AMMOAMOUNT_MD].xPos) * hud_ratio,
	...
	for (int i = MAX_HUD_TICS - 1; i >= 0; i--)
	{
		...
		if (curr_value < inc) // partial tic (alpha it out)
		{
			memcpy(calc_color, ammoTics_md[i].color, sizeof(vec4_t));
			const float percent = curr_value / inc;
			calc_color[3] *= percent;
		}
...
================
*/
static void CG_DrawHealth_md(const float hud_ratio)
{
	vec4_t calc_color;
	const playerState_t* ps = &cg.snap->ps;
	...
	for (int i = MAX_HUD_TICS - 1; i >= 0; i--)
	{
		...
		if (curr_value < inc) // partial tic (alpha it out)
		{
			memcpy(calc_color, healthTics_md[i].color, sizeof(vec4_t));
			const float percent = curr_value / inc;
			calc_color[3] *= percent; // Fade it out
		}
		...
		{
			CG_DrawPic(
				healthTics_md[i].xPos - 4.5 * hud_ratio,
				healthTics_md[i].yPos + 7,
				healthTics_md[i].width * hud_ratio,
		...
		{
			CG_DrawPic(
				healthTics_md[i].xPos * hud_ratio,
	...
	if (cg.snap->ps.weapon == WP_SABER)
	{
		...
		if (cg_SerenityJediEngineHudMode.integer == 3)
		{
			...
			CG_DrawNumField(
				otherHUDBits[OHB_HEALTHAMOUNT_MD].xPos - 4.5 * hud_ratio,
				otherHUDBits[OHB_HEALTHAMOUNT_MD].yPos + 7,
				3,
				ps->stats[STAT_HEALTH],
				otherHUDBits[OHB_HEALTHAMOUNT_MD].width * hud_ratio,
		...
		else
		{
			...
			CG_DrawNumField(
				otherHUDBits[OHB_HEALTHAMOUNT_MD].xPos * hud_ratio,
	...
	else
	{
		...
		if (cg_SerenityJediEngineHudMode.integer == 3)
		{
			...
			CG_DrawNumField(
				otherHUDBits[OHB_HEALTHAMOUNTALT].xPos - 4.5 * hud_ratio,
				otherHUDBits[OHB_HEALTHAMOUNTALT].yPos + 7,
				3,
				ps->stats[STAT_HEALTH],
		...
		else
		{
			...
			CG_DrawNumField(
				otherHUDBits[OHB_HEALTHAMOUNTALT].xPos * hud_ratio,
...
================
*/
static void CG_DrawArmor_md(const float hud_ratio)
{
	vec4_t calc_color;
	const playerState_t* ps = &cg.snap->ps;
	...
	for (int i = MAX_HUD_TICS - 1; i >= 0; i--)
	{
		...
		if (curr_value < inc) // partial tic (alpha it out)
		{
			memcpy(calc_color, armorTics_md[i].color, sizeof(vec4_t));
			const float percent = curr_value / inc;
			calc_color[3] *= percent;
		}
		...
		if (cg_SerenityJediEngineHudMode.integer == 3)
		{
			if (i == MAX_HUD_TICS - 1 && curr_value < inc)
			{
				...
				{
					CG_DrawPic(
						armorTics_md[i].xPos - 4.5 * hud_ratio,
						armorTics_md[i].yPos + 7,
						armorTics_md[i].width * hud_ratio,
	...
	if (cg.snap->ps.weapon == WP_SABER)
	{
		...
		else
		{
			if (cg_SerenityJediEngineHudMode.integer == 3)
			{
				...
				CG_DrawNumField(
					otherHUDBits[OHB_ARMORAMOUNT_MD].xPos - 4.5 * hud_ratio,
					otherHUDBits[OHB_ARMORAMOUNT_MD].yPos + 7,
					3,
					ps->stats[STAT_ARMOR],
					otherHUDBits[OHB_ARMORAMOUNT_MD].width * hud_ratio,
			...
			else
			{
				...
				CG_DrawNumField(
					otherHUDBits[OHB_ARMORAMOUNT_MD].xPos * hud_ratio,
