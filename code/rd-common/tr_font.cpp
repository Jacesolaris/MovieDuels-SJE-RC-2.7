...
CFontInfo::CFontInfo(const char* _fontName)
{
	...
	if (com_buildScript->integer == 2)
	{
		...
		if (!bDone)
		{
			...
			{
				char ssTemp[MAX_QPATH];

				sprintf(ssTemp, "fonts/%s.tga", g_SBCSOverrideLanguages[i].m_psName);
				ri.FS_FOpenFileRead(ssTemp, &f, qfalse);
				if (f) ri.FS_FCloseFile(f);

				sprintf(ssTemp, "fonts/%s.fontdat", g_SBCSOverrideLanguages[i].m_psName);
				ri.FS_FOpenFileRead(ssTemp, &f, qfalse);
				if (f) ri.FS_FCloseFile(f);
			}
